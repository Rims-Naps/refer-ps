...
public final class TextDrawingArea extends Raster {
	...
	private void drawChar(byte[] text, int x, int y, int width, int height, int color, int opacity) {
		int newOffset = x + y * width;
		int place1 = width - width;
		int place2 = 0;
		int oldOffset = 0;
		if (y < clipTop) {
			int Height2 = clipTop - y;
			height -= Height2;
			y = clipTop;
			oldOffset += Height2 * width;
			newOffset += Height2 * width;
		}
		if (y + height >= clipBottom) {
			height -= ((y + height) - clipBottom);
		}
		if (x < clipLeft) {
			int toLeft = clipLeft - x;
			width -= toLeft;
			x = clipLeft;
			oldOffset += toLeft;
			newOffset += toLeft;
		...
		setPixelsInfo(width, height, opacity, raster, text, newOffset, oldOffset, place1, place2, color);
	}
    ...
    public void drawCenteredWithOpacity(String text, int color, int yCoord, int xCoord, int alpha) {
        drawTransparentText(color, text, yCoord, 822, xCoord - getTextWidth(text) / 2, alpha);
    }
    ...
    public void drawCenteredText(String text, int x, int y, int color, boolean centered) {
        drawRegularText(centered, x - getTextWidth(text) / 2, color, text, y);
    }
	...
	public void drawRegularText(boolean flag1, int xPos, int j, String s, int yPos) {
		aBoolean1499 = false;
		int l = xPos;
		if (s == null) {
			return;
		...
		for (int i1 = 0; i1 < s.length(); i1++) {
			if (s.charAt(i1) == '@' && i1 + 4 < s.length() && s.charAt(i1 + 4) == '@') {
				...
				if (c != ' ') {
					...
					method392(aByteArrayArray1491[c], xPos + anIntArray1494[c], yPos + anIntArray1495[c], anIntArray1492[c], anIntArray1493[c], j);
				}
				xPos += rsb[c];
			}
		}
		if (aBoolean1499) {
			drawHorizontalLine(yPos + (int) ((double) anInt1497 * 0.69999999999999996D) - 1, 0x800000, xPos - l, l);
		}
	}
	...
	private int getColorByName(String s) {
        ...
        if (s.equals("vag")) {
            return ColorConstants.VANGUARD_PURPLE;
        }
		if (s.equals("str")) {
			aBoolean1499 = true;
		}
	...
	private void method392(byte[] abyte0, int i, int j, int k, int l, int i1) {
		...
		if (i + k >= clipRight) {
			int l2 = ((i + k) - clipRight) + 1;
			k -= l2;
			l1 += l2;
			k1 += l2;
		}
		if (!(k <= 0 || l <= 0)) {
			method393(raster, abyte0, i1, i2, j1, k, l, k1, l1);
		}
	}
	...
	private void method394(int i, int j, byte[] abyte0, int k, int l, int i1, int j1) {
		...
		method395(abyte0, i1, k1, raster, j2, k, i2, l1, j1, i);
	}
	...
	public void drawRightAlignedString(String s, int x, int y, int color) {
		drawRegularText(false, x - method384(s), color, s, y);
	}
}
