...
public class GrandExchange {
	...
	public void drawGrandExchange() {
		...
		if (Client.openInterfaceID == 54700) {
			per4 = Client.spritesMap.get(1158);
			per5 = Client.spritesMap.get(1159);
			per6 = Client.spritesMap.get(1160);
			abort2 = Client.spritesMap.get(1161);
	...
	public void drawUpdate(int id, String type) {
		...
		if (type == "Sell") {
			if (Client.getClient().mouseX() >= x && Client.getClient().mouseX() <= x + 140 && Client.getClient().mouseY() >= y && Client.getClient().mouseY() <= y + 110 && !Client.getClient().menuOpen) {
				SellHover = Client.spritesMap.get(1168);
				if (SellHover != null) {
					SellHover.drawSprite(x, y);
				}
			} else {
				Client.spritesMap.get(1169).drawSprite(x, y);
			}
			Client.spritesMap.get(1170).drawSprite(x + 6, y + 30);
			...
			drawPercentage(id);
			Client.getClient().smallText.method592(0xCC9900, x2, RSInterface.interfaceCache[32000 + id].message, y2 - minus, true);
			Client.getClient().smallText.method592(0xBDBB5B, x2, RSInterface.interfaceCache[33000 + id].message, y2, true);
			Client.getClient().smallText.method592(0xFFFF00, x3, RSInterface.interfaceCache[33100 + id].message, y3, true);
			setHovers(id, false);
			if (Client.spritesMap.get(1145) != null) {
				Client.spritesMap.get(1145).drawSprite(x + 110, y + 39);
				if (Client.getClient().mouseInRegion(x + 112, y + 38, x + 132, y + 60)) {
					Client.spritesMap.get(1146).drawSprite(x + 110, y + 39);
					changeSet(id, false, true);
				}
			}
			String[] name = Client.optimizeText(ItemDef.get(slotItems[id]).name).split(" ");
			int index = 0;
			for (String n : name) {
				int xDraw = x + 77;
			...
			if (Client.getClient().mouseX() >= x && Client.getClient().mouseX() <= x + 140 && Client.getClient().mouseY() >= y && Client.getClient().mouseY() <= y + 110 && !Client.getClient().menuOpen) {
				BuyHover = Client.spritesMap.get(1167);
				if (BuyHover != null) {
					BuyHover.drawSprite(x, y);
				}
			} else {
				Client.spritesMap.get(1171).drawSprite(x, y);
			}
			Client.spritesMap.get(1170).drawSprite(x + 6, y + 30);
			if (slotItems[id] > 0 && ItemDef.getSprite(slotItems[id], 1, 0) != null) {
				ItemDef.getSprite(slotItems[id], 1, 0).drawSprite(x + 9, y + 32);
			...
			if (Client.getClient().mouseX() >= x && Client.getClient().mouseX() <= x + 140 && Client.getClient().mouseY() >= y && Client.getClient().mouseY() <= y + 110 && !Client.getClient().menuOpen) {
				buySubmitHover = Client.spritesMap.get(1172);
				if (buySubmitHover != null) {
					buySubmitHover.drawSprite(x, y);
				}
			} else {
				Client.spritesMap.get(27).drawSprite(x, y);
			}
			setGrandExchange(id, false);
			...
			if (Client.getClient().mouseX() >= x && Client.getClient().mouseX() <= x + 140 && Client.getClient().mouseY() >= y && Client.getClient().mouseY() <= y + 110 && !Client.getClient().menuOpen) {
				sellSubmitHover = Client.spritesMap.get(1173);
				if (sellSubmitHover != null) {
					sellSubmitHover.drawSprite(x, y);
				}
			} else {
				Client.spritesMap.get(28).drawSprite(x, y);
			}
			setGrandExchange(id, false);
			changeSet(id, false, false);
			Client.getClient().smallText.method592(0xCC9900, x2, RSInterface.interfaceCache[32000 + id].message, y2 - minus, true);
			...
			if (Client.getClient().mouseX() >= x && Client.getClient().mouseX() <= x + 140 && Client.getClient().mouseY() >= y && Client.getClient().mouseY() <= y + 110 && !Client.getClient().menuOpen) {
				BuyHover = Client.spritesMap.get(1172);
				if (BuyHover != null) {
					BuyHover.drawSprite(x, y);
				}
			} else {
	...
	public void drawPercentage(int id) {
		per0 = Client.spritesMap.get(1162);
		per1 = Client.spritesMap.get(1163);
		per2 = Client.spritesMap.get(1164);
		per3 = Client.spritesMap.get(1165);
		abort1 = Client.spritesMap.get(1166);
		int x = 0;
		int y = 0;
		boolean fixed = (GameFrame.getScreenMode() == ScreenMode.FIXED);
		switch (id) {
	...
	public void displayItemSearch() {
		...
		try {
			if (searching && Client.getClient().inputDialogState == 3) {
				...
				Client.spritesMap.get(1149).drawSprite(xPosOffset, yPosOffset);
				Raster.setDrawingArea(121 + yPosOffset, 8, 512, 7);
				Client.spritesMap.get(1170).drawSprite(18, 18 + yPosOffset);
				for (int j = 0; j < totalItemResults; j++) {
					...
					if (yPos > 0 && yPos < 210) {
						...
						Client.getClient().boldText.method591(Client.capitalizeFirstChar(n), 78, 0xA05A00, yPos + yPosOffset + (totalItemResults < 8 ? 6 : 0));
						if (x > 74 && x < 495 && y > ((GameFrame.getScreenMode() == ScreenMode.FIXED) ? 338 : Client.clientHeight - 165) + yPos - 13 + (totalItemResults < 8 ? 6 : 0) && y < ((GameFrame.getScreenMode() == ScreenMode.FIXED) ? 338 : Client.clientHeight - 165) + yPos + 2 + (totalItemResults < 8 ? 6 : 0)) {
							Raster.fillRect(0x807660, yPos - 12 + yPosOffset + (totalItemResults < 8 ? 6 : 0), 424, 15, 60, 75);
				...
				Raster.drawPixels(113, 8 + yPosOffset, 74, 0x807660, 2);
				Raster.defaultDrawingAreaSize();
				if (totalItemResults > 8) {
					Client.getClient().drawScrollbar(114, itemResultScrollPos, 8 + yPosOffset, 496 + xPosOffset, totalItemResults * 14, false, false);
				...
				if (Client.getClient().amountOrNameInput.length() == 0) {
					Client.getClient().boldText.drawCenteredText(0xA05A00, 259, "Grand Exchange Item Search", 30 + yPosOffset, false);
					...
					Client.getClient().smallText.drawCenteredText(0xA05A00, 259, "Then, simply select the item you want from the results on the display.", 80 + 15 + yPosOffset, false);
					// boldText.drawText(0xffffff, amountOrNameInput +
					// "*", 32, 133);
					showMatches = false;
				}
				if (totalItemResults == 0 && showMatches) {
					Client.getClient().smallText.drawCenteredText(0xA05A00, 259, "No matching items found", 80 + yPosOffset, false);
				}
				Raster.fillRect(0x807660, 121 + yPosOffset, 506, 15, 120, 7);// box
				// boldText.drawText(0, "<img=8>", 133, 12);
				Client.getClient().boldText.method591(Client.getClient().amountOrNameInput + "*", 28 + xPosOffset, 0xffffff, 133 + yPosOffset);
				// boldText.drawText(0xffffff, amountOrNameInput + "*",
				// 133, 122);
				Raster.drawLine(121 + yPosOffset, 0x807660, 506, 7);// line
				// drawClose(496, 122, 496, 345 + 112, 496 + 19, 361 + 112);
				// drawClose(496, 122, 496, 345 + 112, 496 + 19, 361 + 112);
				// drawClose(496, 122, 496, 345 + 112, 496 + 19, 361 + 112);
			}
			Client.spritesMap.get(1148).drawSprite(11, 122 + yPosOffset);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void itemSearch(String n) {
		...
		for (int id = 0; id < ItemDef.totalItems; id++) {
			ItemDef item = ItemDef.get(id);
			if (item == null) {
				continue;
			}
			if (item.certTemplateID > -1 || item.name == null || item.name == "Picture" || item.certID == 18786 || item.name == "Null" || item.name.toLowerCase().contains("coins") || item.value <= 0 || item.id == 4178 || item.id == 14661/*|| item.untradeable*/) {
				continue;
			}
	...
	public void buildItemSearch(int mouseY) {
		...
		for (int idx = 0; idx < 100; idx++) {
			...
			if (mouseY > textY - 14 && mouseY <= textY && Client.getClient().mouseX() > 74 && Client.getClient().mouseX < 495) {
