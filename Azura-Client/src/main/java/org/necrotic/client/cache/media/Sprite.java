...
public class Sprite extends Raster {
	...
	/*	private static MRUNodes<String> ims = new ArrayList<String>();
	static int  c = 0;*/
...
/*if(ims.contains(img)) {
	...
	public static BufferedImage cropImage2(BufferedImage src, int cropFromTop, int cropFromBottom) {
	...
	public Sprite getResizedSprite(Sprite spriteParam, int width, int height) {
		if (width == 0) {
			width = 1;
		}
		if (height == 0) {
	...
	public void drawSprite1(int i, int j, int k) {
		...
		int j2 = 0;
		if (j < Raster.clipTop) {
			int k2 = Raster.clipTop - j;
			k1 -= k2;
			j = Raster.clipTop;
			j1 += k2 * l1;
			i1 += k2 * Raster.width;
		}
		if (j + k1 > Raster.clipBottom) {
			k1 -= (j + k1) - Raster.clipBottom;
		}
		if (i < Raster.clipLeft) {
			int l2 = Raster.clipLeft - i;
			l1 -= l2;
			i = Raster.clipLeft;
			j1 += l2;
			i1 += l2;
		...
		if (i + l1 > Raster.clipRight) {
			int i3 = (i + l1) - Raster.clipRight;
			l1 -= i3;
			j2 += i3;
			i2 += i3;
		}
		if (l1 > 0 && k1 > 0) {
			block_copy_alpha(j1, l1, Raster.raster, myPixels, j2, k1, i2, k, i1);
		}
	}
	...
	public void drawAdvancedSprite(int i, int j, int alpha) {
		...
		if (!(l1 <= 0 || k1 <= 0)) {
			drawAlphaGlow(j1, l1, Raster.raster, myPixels, j2, k1, i2, k, i1);
		}
	}

	/*public void drawAdvancedSprite(int i, int j, int k) {
		int i1 = i + j * TextDrawingArea.width;
		int j1 = 0;
		int k1 = myHeight;
		int l1 = myWidth;
		int i2 = TextDrawingArea.width - l1;
		int j2 = 0;
		if (j < TextDrawingArea.topY) {
			int k2 = TextDrawingArea.topY - j;
			k1 -= k2;
			j = TextDrawingArea.topY;
			j1 += k2 * l1;
			i1 += k2 * TextDrawingArea.width;
		}
		if (j + k1 > TextDrawingArea.bottomY)
			k1 -= (j + k1) - TextDrawingArea.bottomY;
		if (i < TextDrawingArea.topX) {
			int l2 = TextDrawingArea.topX - i;
			l1 -= l2;
			i = TextDrawingArea.topX;
			j1 += l2;
			i1 += l2;
			j2 += l2;
			i2 += l2;
		}
		if (i + l1 > TextDrawingArea.bottomX) {
			int i3 = (i + l1) - TextDrawingArea.bottomX;
			l1 -= i3;
			j2 += i3;
			i2 += i3;
		}
		if (!(l1 <= 0 || k1 <= 0)) {
			drawAlphaSprite(j1, l1, TextDrawingArea.pixels, myPixels, j2, k1, i2, k, i1);
		}
	}*/
	...
	public void drawAdvancedSprite(int i, int j) {
		...
		if (!(l1 <= 0 || k1 <= 0)) {
			drawAlphaSprite(j1, l1, Raster.raster, myPixels, j2, k1, i2, k, i1);
		}
	}
	...
	public void drawARGBSprite(int xPos, int yPos, int alpha) {
		...
		int j2 = 0;
		if (yPos < Raster.clipTop) {
			int k2 = Raster.clipTop - yPos;
			spriteHeight -= k2;
			yPos = Raster.clipTop;
			j1 += k2 * spriteWidth;
			i1 += k2 * Raster.width;
		}
		if (yPos + spriteHeight > Raster.clipBottom) {
			spriteHeight -= (yPos + spriteHeight) - Raster.clipBottom;
		}
		if (xPos < Raster.clipLeft) {
			int l2 = Raster.clipLeft - xPos;
			spriteWidth -= l2;
			xPos = Raster.clipLeft;
			j1 += l2;
			i1 += l2;
			j2 += l2;
			i2 += l2;
		}
		if (xPos + spriteWidth > Raster.clipRight) {
			int i3 = (xPos + spriteWidth) - Raster.clipRight;
			spriteWidth -= i3;
			j2 += i3;
			i2 += i3;
		}
		if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
			renderARGBPixels(spriteWidth, spriteHeight, myPixels, Raster.raster, i1, alphaValue, j1, j2, i2);
		}
	}
	...
	public void method346(int i, int j) {
		...
		int i2 = 0;
		if (j < Raster.clipTop) {
			int j2 = Raster.clipTop - j;
			j1 -= j2;
			j = Raster.clipTop;
			i1 += j2 * k1;
			l += j2 * Raster.width;
		}
		if (j + j1 > Raster.clipBottom) {
			j1 -= (j + j1) - Raster.clipBottom;
		}
		if (i < Raster.clipLeft) {
			int k2 = Raster.clipLeft - i;
			k1 -= k2;
			i = Raster.clipLeft;
			i1 += k2;
			l += k2;
		...
		if (i + k1 > Raster.clipRight) {
			int l2 = (i + k1) - Raster.clipRight;
			k1 -= l2;
			i2 += l2;
			l1 += l2;
		}
		if (k1 <= 0 || j1 <= 0) {
			...
			block_copy(l, k1, j1, i2, i1, l1, myPixels, Raster.raster);
		}
	}
	...
	public void drawSprite(int xPos, int yPos) {
		...
		int i2 = 0;
		if (yPos < Raster.clipTop) {
			int j2 = Raster.clipTop - yPos;
			j1 -= j2;
			yPos = Raster.clipTop;
			i1 += j2 * k1;
			l += j2 * Raster.width;
		}
		if (yPos + j1 > Raster.clipBottom) {
			j1 -= (yPos + j1) - Raster.clipBottom;
		}
		if (xPos < Raster.clipLeft) {
			int k2 = Raster.clipLeft - xPos;
			k1 -= k2;
			xPos = Raster.clipLeft;
			i1 += k2;
			l += k2;
		...
		if (xPos + k1 > Raster.clipRight) {
			int l2 = (xPos + k1) - Raster.clipRight;
			k1 -= l2;
			i2 += l2;
			l1 += l2;
		}
		if (!(k1 <= 0 || j1 <= 0)) {
			try {
				block_copy_trans(Raster.raster, myPixels, i1, l, k1, j1, l1, i2);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public void drawSprite3(int x, int y, int opacity) {
		...
		int srcStep = 0;
		if (y < Raster.clipTop) {
			int trimHeight = Raster.clipTop - y;
			height -= trimHeight;
			y = Raster.clipTop;
		...
		if (y + height > Raster.clipBottom) {
			height -= (y + height) - Raster.clipBottom;
		}
		if (x < Raster.clipLeft) {
			int trimLeft = Raster.clipLeft - x;
			width -= trimLeft;
			x = Raster.clipLeft;
			srcOffset += trimLeft;
			destOffset += trimLeft;
		...
		if (x + width > Raster.clipRight) {
			int trimRight = (x + width) - Raster.clipRight;
			width -= trimRight;
			srcStep += trimRight;
			destStep += trimRight;
		}
		if (!((width <= 0) || (height <= 0))) {
			setPixels(width, height, Raster.raster, myPixels, alpha, destOffset, srcOffset, destStep, srcStep);
		}
	}
	...
	public void drawSprite(int i, int k, int color) {
		...
		int i2 = 0;
		if (k < Raster.clipTop) {
			int j2 = Raster.clipTop - k;
			j1 -= j2;
			k = Raster.clipTop;
			i1 += j2 * k1;
			l += j2 * Raster.width;
		}
		if (k + j1 > Raster.clipBottom) {
			j1 -= (k + j1) - Raster.clipBottom;
		}
		if (i < Raster.clipLeft) {
			int k2 = Raster.clipLeft - i;
			k1 -= k2;
		...
		if (!(k1 <= 0 || j1 <= 0)) {
			block_copy_trans(Raster.raster, tempArray, i1, l, k1, j1, l1, i2);
		}
	}
	...
	public void draw(Background background, int i, int j) {
		...
		int l1 = 0;
		if (i < Raster.clipTop) {
			int i2 = Raster.clipTop - i;
			i1 -= i2;
			i = Raster.clipTop;
			l += i2 * j1;
			k += i2 * Raster.width;
		}
		if (i + i1 > Raster.clipBottom) {
			i1 -= (i + i1) - Raster.clipBottom;
		}
		if (j < Raster.clipLeft) {
			int j2 = Raster.clipLeft - j;
			j1 -= j2;
			j = Raster.clipLeft;
			l += j2;
			k += j2;
		...
		if (j + j1 > Raster.clipRight) {
			int k2 = (j + j1) - Raster.clipRight;
			j1 -= k2;
			l1 += k2;
			k1 += k2;
		}
		if (!(j1 <= 0 || i1 <= 0)) {
			block_copy_mask(myPixels, j1, background.imgPixels, i1, Raster.raster, 0, k1, k, l1, l);
		}
	}
	...
	public void drawSpriteWithOpacity(int xPos, int yPos, int o) {
		...
		if (yPos + k1 > Raster.clipBottom) {
			k1 -= (yPos + k1) - Raster.clipBottom;
		}
		if (xPos < Raster.clipLeft) {
			int l2 = Raster.clipLeft - xPos;
			l1 -= l2;
		...
		if (xPos + l1 > Raster.clipRight) {
			int i3 = (xPos + l1) - Raster.clipRight;
			l1 -= i3;
			j2 += i3;
			i2 += i3;
		}
		if (!(l1 <= 0 || k1 <= 0)) {
			block_copy_alpha(j1, l1, Raster.raster, myPixels, j2, k1, i2, opacity, i1);
		}
	}
