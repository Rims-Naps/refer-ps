...
public final class Background extends Raster {
	...
	public Background(Archive streamLoader, String s, int i, boolean texture) {
		Stream stream = new Stream(streamLoader.getDataForName(s + ".dat"));
		Stream stream_1 = new Stream(streamLoader.getDataForName("index.dat"));
		stream_1.position = texture ? stream.getInt() : stream.getUnsignedShort();
		maxWidth = stream_1.getUnsignedShort();
		maxHeight = stream_1.getUnsignedShort();
		int j = texture ? stream_1.getShort() : stream_1.getUnsignedByte();
		palette = new int[j];
		for (int k = 0; k < j - 1; k++) {
		...
		imgPixels = new int[j1];
		if (i1 == 0) {
			for (int k1 = 0; k1 < j1; k1++) {
				imgPixels[k1] = texture ? stream.getShort() : stream.getSignedByte();
			}
		...
		if (i1 == 1) {
			for (int l1 = 0; l1 < imgWidth; l1++) {
				for (int i2 = 0; i2 < imgHeight; i2++) {
					imgPixels[l1 + i2 * imgWidth] = texture ? stream.getShort() : stream.getSignedByte();
				}
	...
	public void method356() {
		maxWidth /= 2;
		maxHeight /= 2;
		int[] abyte0 = new int[maxWidth * maxHeight];
		int i = 0;
	...
	public void method361(int i, int k) {
		...
		if (!(k1 <= 0 || j1 <= 0)) {
			method362(j1, Raster.raster, imgPixels, l1, l, k1, i1, palette, i2);
		}
	}

	private void method362(int i, int[] ai, int[] abyte0, int j, int k, int l, int i1, int[] ai1, int j1) {
		int k1 = -(l >> 2);
		l = -(l & 3);
