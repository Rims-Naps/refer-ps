...
import org.necrotic.client.cache.media.CursorData;
import org.necrotic.client.cache.media.rsinterface.dropdowns.impl.Keybinding;
import org.necrotic.client.media.ProducingGraphicsBuffer;
...
public class GameRenderer extends Applet implements Runnable, MouseListener, MouseMotionListener, KeyListener, FocusListener, WindowListener, MouseWheelListener {
	...
	public final void keyPressed(KeyEvent keyEvent) {
		...
		if (keyEvent.isControlDown()) {
			if (keyCode == KeyEvent.VK_T) {
				...
				sendCommand("progression");
				keyEvent.consume();
				return;

			}
			else if (keyCode == KeyEvent.VK_E) {
				sendCommand("enchanter");
				keyEvent.consume();
				return;
	...
	/*	if (Client.inMoveButton || dragging) {
			if (dragging && (preDragX > 0 || preDragX > 0)) {
				if (Client.openInterfaceID != -1) {
					InterfaceMovementManager.moveInterface(Client.openInterfaceID, (x - preDragX), (y - preDragY));
				}
...
*/
	...
	public void mouseWheelMoved(MouseWheelEvent e) {
		...
		if (mouseX > xPos && mouseX < xPos + 512 && mouseY > yPos && mouseY < yPos + 140 && !Client.instance.chatArea.componentHidden()) {
			...
			if (zoom && Client.openInterfaceID == -1) {
				...
				int max_zoom_1 = (GameFrame.getScreenMode() == ScreenMode.FIXED ? -700 : -800);
				if (Client.cameraZoom < max_zoom_1) {
					Client.cameraZoom = max_zoom_1;
				}
				if (Client.cameraZoom > Client.MAX_CAMERA_ZOOM) {
					Client.cameraZoom = Client.MAX_CAMERA_ZOOM;
				}
			...
			Client.instance.setInputTaken(true);*/
			boolean zoom = GameFrame.getScreenMode() == ScreenMode.FIXED ? (mouseX < 512) : (mouseX < Client.clientWidth - 200);
			boolean mapZoom = GameFrame.getScreenMode() == ScreenMode.FIXED ? (mouseX > 526 && mouseY < 150) : ((Client.clientWidth - mouseX) < 180 && (Client.clientHeight - mouseY) > 635);
			if (mapZoom) {
				Client.instance.minimapZoom += rotation * 35;

				if (Client.instance.minimapZoom > 60) {
					Client.instance.minimapZoom = 60;
				}

				if (Client.instance.minimapZoom < -70) {
					Client.instance.minimapZoom = -70;
				}

			}
			if (zoom && Client.openInterfaceID == -1) {
				Client.cameraZoom += rotation * 120;

				if (Client.cameraZoom < -20 && !(GameFrame.getScreenMode() == ScreenMode.FIXED))
					Client.cameraZoom = -20;
				else if (Client.cameraZoom < -50 && GameFrame.getScreenMode() == ScreenMode.FIXED)
					Client.cameraZoom = -50;
				if (Client.cameraZoom > 1500)
					Client.cameraZoom = 1500;
			}
	...
	public boolean handleInterfaceScrolling(MouseWheelEvent event) {
       ...
       */
        ...
        int tabInterfaceID = -1;
		/* Tab interface scrolling */
        if (!Client.instance.tabArea.componentHidden()) {
            tabInterfaceID = Client.tabInterfaceIDs[Client.tabID];
        }
		if (tabInterfaceID == 11000) {
			tabInterfaceID = 1151;
		}
	...
	@Override
	public void run() {
		if (gameFrame != null) gameFrame.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				//if(Client.loggedIn) {
					int result = JOptionPane.showConfirmDialog(gameFrame,
				            "Are you sure you want to exit Athens ?", "Exit Confirmation",
				            JOptionPane.YES_NO_OPTION);
				        if (result == JOptionPane.YES_OPTION) {
				        	gameFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                            Save.settings(Client.instance);
				        	System.exit(0);
