...
public class MathUtils {
    ...
    public static double calculatePercentage(int current, int max) {
        float percentFloat = (float)current / max;
        float outValueFloat = (100 * percentFloat);
        return Math.round(outValueFloat);
    }

    public static double calculateSpritesToDrawFromPercent(int percent, int spriteWidth) {
        float percentFloat = (float)percent * spriteWidth;
        float outValueFloat = (percentFloat / 100);
        return Math.round(outValueFloat);
    }
}
