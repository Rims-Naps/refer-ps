...
public class RSFontSystem extends Raster {
	...
	public static String handleOldSyntax(String text) {
		text = text.replaceAll("@vol@", "<col=af70c3>");
		...
		if (Configuration.SMILIES_ENABLED && !text.contains(":store") && !text.contains(":Discord")) {
			...
			if (character == 60) {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								if (imageId >= 1 && imageId <= 20) { //Player icons
									imageId = 827 + imageId;
								} else if (imageId >= 21 && imageId <= 31) { //Smilies
									imageId = 838 + imageId;
									offsetY += 3;
								}
				...
				if (startIndex == -1) {
					...
					if (strikethroughColor != -1) {
						Raster.drawHorizontalLine(drawY + (int) (baseCharacterHeight * 0.7), strikethroughColor, lineWidth, drawX);
					}
					if (underlineColor != -1) {
						Raster.drawHorizontalLine(drawY + baseCharacterHeight, underlineColor, lineWidth, drawX);
					}
					drawX += lineWidth;
				}
			}
    ...
    public void drawCenteredText(String text, int x, int y, int color, int shadow) {
        if (text != null) {
            setColorAndShadow(color, shadow);
            text = handleOldSyntax(text);
            drawBasicString(text, x - getTextWidth(text) / 2, y, true);
        }
    }
	...
	private void drawCharacter(int character, int i_35_, int i_36_, int i_37_, int i_38_, int i_39_, boolean bool) {
		...
		if (i_36_ < Raster.clipTop) {
			int i_44_ = Raster.clipTop - i_36_;
			i_38_ -= i_44_;
			i_36_ = Raster.clipTop;
			i_43_ += i_44_ * i_37_;
			i_40_ += i_44_ * Raster.width;
		}

		if (i_36_ + i_38_ > Raster.clipBottom) {
			i_38_ -= i_36_ + i_38_ - Raster.clipBottom;
		}

		if (i_35_ < Raster.clipLeft) {
			int i_45_ = Raster.clipLeft - i_35_;
			i_37_ -= i_45_;
			i_35_ = Raster.clipLeft;
			i_43_ += i_45_;
			i_40_ += i_45_;
			i_42_ += i_45_;
			i_41_ += i_45_;
		...
		if (i_35_ + i_37_ > Raster.clipRight) {
			int i_46_ = i_35_ + i_37_ - Raster.clipRight;
			i_37_ -= i_46_;
			i_42_ += i_46_;
			i_41_ += i_46_;
		}
		...
		if (i_37_ > 0 && i_38_ > 0) {
			try {
				createCharacterPixels(Raster.raster, fontPixels[character], i_39_, i_43_, i_40_, i_37_, i_38_, i_41_, i_42_);
			} catch (Exception ex) {
			}
		}
	}
	...
	private void drawTransparentCharacter(int i, int i_11_, int i_12_, int i_13_, int i_14_, int i_15_, int i_16_, boolean bool) {
		...
		int i_19_ = 0;
		int i_20_ = 0;
		if (i_12_ < Raster.clipTop) {
			int i_21_ = Raster.clipTop - i_12_;
			i_14_ -= i_21_;
			i_12_ = Raster.clipTop;
			i_20_ += i_21_ * i_13_;
			i_17_ += i_21_ * Raster.width;
		}
		if (i_12_ + i_14_ > Raster.clipBottom) {
			i_14_ -= i_12_ + i_14_ - Raster.clipBottom;
		}
		if (i_11_ < Raster.clipLeft) {
			int i_22_ = Raster.clipLeft - i_11_;
			i_13_ -= i_22_;
			i_11_ = Raster.clipLeft;
			i_20_ += i_22_;
			i_17_ += i_22_;
			i_19_ += i_22_;
			i_18_ += i_22_;
		}
		if (i_11_ + i_13_ > Raster.clipRight) {
			int i_23_ = i_11_ + i_13_ - Raster.clipRight;
			i_13_ -= i_23_;
			i_19_ += i_23_;
			i_18_ += i_23_;
		}
		if (i_13_ > 0 && i_14_ > 0) {
			createTransparentCharacterPixels(Raster.raster, fontPixels[i], i_15_, i_20_, i_17_, i_13_, i_14_, i_18_, i_19_, i_16_);
		}
	}
						...
						if (effectString.equals(startEffect)) {
							...
							if (effectString.startsWith(startImage)) {
								try {// <img=
									...
									int iconWidth = Client.spritesMap.get(iconId).myWidth + 2;
