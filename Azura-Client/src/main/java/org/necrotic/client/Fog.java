...
public class Fog {
    ...
    public static void draw(int rgb) {
        if(Raster.raster == null || Raster.depthBuffer == null) {
            return;
        }
        begin = ClientSettings.fogStartValue;
        for (int index = 0; index < Raster.raster.length; index++) {
        	Raster.raster[index] = blend(Raster.raster[index], rgb, (Raster.depthBuffer[index] - begin) / length);
        }
    }
