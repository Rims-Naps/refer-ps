...
public final class Projectile extends Renderable {
	...
	public Model getRotatedModel() {
		...
		Model model_1 = new Model(true, AnimationSkeleton.isNullFrame(j), false, model);
		if (j != -1) {
			model_1.createBones();
		...
		model_1.light(64 + spotAnim.ambient, 5050 + spotAnim.contrast, -90, -580, -90, true);
		return model_1;
			/*
		Model model = spotAnim.getModel();
		if (model == null)
		...
		if (spotAnim.resizeX != 128 || spotAnim.resizeY != 128) {
			model_1.scaleT(spotAnim.resizeX, spotAnim.resizeX, spotAnim.resizeY);
		}
	...
	public Projectile(int i, int j, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2) {
		aBoolean1579 = false;
		spotAnim = AnimatedGraphic.cache[l2];
		anInt1597 = k1;
		anInt1580 = j2;
		anInt1581 = i2;
		anInt1582 = l1;
