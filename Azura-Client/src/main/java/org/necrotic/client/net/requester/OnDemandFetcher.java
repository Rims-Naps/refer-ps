package org.necrotic.client.net.requester;
...
import org.necrotic.client.collection.NodeSubList;
...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
	...
	public int getChecksum(int type, int id) {
		int crc = 0;
		byte[] data = clientInstance.decompressors[type + 1].read(id);
		if (data != null) {
	...
	public void dump() {
		...
		for (int element : mapIndices2) {
			try {
				byte[] abyte = clientInstance.decompressors[4].read(element);
	...
	private final NodeList extraFilesList;
	...
	private final NodeList incompleteList;
	...
	private final NodeList next;
	private OnDemandRequest current;
	private final NodeList loadRequestList;
	...
	public OnDemandFetcher() {
		...
		extraFilesList = new NodeList();
		running = true;
		waiting = false;
		incompleteList = new NodeList();
		gzipInputBuffer = new byte[3000000];
		queue = new NodeSubList();
		crcs = new int[5][];
		next = new NodeList();
		loadRequestList = new NodeList();
	}
	...
	private void checkReceived() {
		...
		while (onDemandData != null) {
			...
			if (clientInstance.decompressors[0] != null) {
				abyte0 = clientInstance.decompressors[onDemandData.getDataType() + 1].read(onDemandData.getId());
			}
	...
	public void setExtraPriority(byte byte0, int i, int j) {
		try {
			...
			byte[] abyte0 = clientInstance.decompressors[i + 1].read(j);
		...
		try {
			...
			if (expectedSize > 0 && available >= expectedSize) {
				...
				if (expectedSize + completedSize >= buf.length && current != null) {
					if (clientInstance.decompressors[0] != null) {
						clientInstance.decompressors[current.getDataType() + 1].write(buf.length, buf, current.getId());
					}
	...
	public void start(Archive streamLoader, Client client1) {
		...
		for (int index = 0; index < 4; index++) {
			byte[] abyte1 = streamLoader.getDataForName(as1[index]);
