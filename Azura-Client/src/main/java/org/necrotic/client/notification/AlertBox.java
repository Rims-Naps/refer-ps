...
public class AlertBox {
	...
	public void draw() {
		...
		// TextDrawingArea.drawAlphaGradient2(i, j + 25, width, getHeight() - 25,
		// paramColor.darker().getRGB(), paramColor.brighter().getRGB(), opacity);

		Raster.drawRoundedRectangle(i, j + 5, width, getHeight() - 5, color, opacity, true, false);

		// TextDrawingArea.drawRectangle(j, 25, opacity, paramColor.darker().getRGB(),
		// width, i);
		if (!closing) {
