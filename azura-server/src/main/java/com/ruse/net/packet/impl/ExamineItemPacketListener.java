...
public class ExamineItemPacketListener implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
		...
		int tab = Bank.getTabForItem(player, 995);
		if (item == ItemDefinition.COIN_ID) {
			int amount = player.getInventory().getAmount(item);
			if (player.getInterfaceId() == 5292) {
				player.msgPurp("" + player.getBank(tab).getAmount(995) + "x " + ItemDefinition.forId(item).getName() + " in your bank!");
				if (player.getInventory().getAmount(ItemDefinition.COIN_ID) >= 1) {
					player.msgPurp(amount + "x " + ItemDefinition.forId(item).getName() + " in your inventory");
				}
			} else {
				player.msgPurp(amount + "x " + ItemDefinition.forId(item).getName());
			}
			return;
		}
...
//		if (itemDef != null) {
//			player.getPacketSender().sendString(39802, itemDef.getName());
//			player.getPacketSender().sendString(39803, itemDef.getDescription());
//			player.getPacketSender().sendString(39804, "");
//			player.getPacketSender().sendItemOnInterface(39805, itemDef.getId(), 1);
//			player.getPacketSender().sendInterface(39800);
//		}
//			for (Skill skill : Skill.values()) {
//				if (itemDef.getRequirement()[skill.ordinal()] > player.getSkillManager().getMaxLevel(skill)) {
//					player.getPacketSender().sendMessage("@red@WARNING: You need " + new StringBuilder().append(skill.getName().startsWith("a") || skill.getName().startsWith("e") || skill.getName().startsWith("i") || skill.getName().startsWith("o") || skill.getName().startsWith("u") ? "an " : "a ") + Misc.formatText(skill.getName()) + " level of at least " + itemDef.getRequirement()[skill.ordinal()] + " to wear this.");
//				}
		//	}
		}
	}
