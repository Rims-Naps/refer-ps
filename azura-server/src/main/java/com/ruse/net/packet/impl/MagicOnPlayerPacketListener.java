	...
	public void handleMessage(Player player, Packet packet) {
		...
		if (spell == CombatSpells.VENGEANCE_OTHER.getSpell()) {
			...
			if (player.getSkillManager().getCurrentLevel(Skill.MAGIC) < 93) {
				...
				player.getPacketSender().sendMessage("That player already has CorruptRaid active.");
			} else if (!player.getLastVengeance().elapsed(30000)) {
				player.getPacketSender().sendMessage("You can only cast a CorruptRaid once every 30 seconds.");
			} else {
				...
				player.getPacketSender().sendMessage("You cast CorruptRaid on " + attacked.getUsername() + ".");
				attacked.getPacketSender().sendMessage("Your were given CorruptRaid by " + player.getUsername());
