...
public class SwitchItemSlotPacketListener implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
				...
				if (player.isBanking() && player.getInterfaceId() == 5292) {
                    if (player.getViewing().equals(Player.INTERFACES.GROUP_BANK)) {
                        ...
                        if (player.getGroupBankSearchingAttribtues().isSearchingBank()) {
                            GroupIronmanBank bank = player.getGroupBankSearchingAttribtues().getSearchedBank();
                            if (bank != null) {
                                if (player.swapMode()) {
                            ...
                            if (player.swapMode()) {
                                player.getGroupIronmanBank(player.getCurrentGroupBankTab()).shiftSwap(fromSlot, toSlot);
                            } else {
                                player.getGroupIronmanBank(player.getCurrentGroupBankTab()).swap(fromSlot, toSlot);
                            }
                            player.getGroupIronmanBank(player.getCurrentGroupBankTab()).open(player, false, false);
                        }
                        return;
                    } else {
                        if (fromSlot == toSlot)
		...
		if (player.isBanking() && player.getInterfaceId() == 5292) {
            ...
            if (player.getViewing().equals(Player.INTERFACES.GROUP_BANK)) {
                for (int i = 0; i < GroupIronmanBank.BANK_TAB_INTERFACES.length; i++) {
                    toBankTab = GroupIronmanBank.BANK_TAB_INTERFACES[i][0] == interfaceId;
                    Item item = new Item(player.getGroupIronmanBank(player.getCurrentGroupBankTab()).getItems()[fromSlot].getId(),
                        player.getGroupIronmanBank(player.getCurrentGroupBankTab()).getItems()[fromSlot].getAmount());

                    int realAmt = player.getGroupIronmanBank(player.getCurrentGroupBankTab()).getAmount(item.getId());
                    if (realAmt == 0) {
                        return;
                    }

                    if (!player.getGroupIronmanBank(player.getCurrentGroupBankTab()).contains(item.getId()) || realAmt < item.getAmount())
                        return;
                    if (toBankTab) {
                        if (player.getBankSearchingAttribtues().isSearchingBank()) {
                        ...
                        int bankTab = GroupIronmanBank.BANK_TAB_INTERFACES[i][1];
                        int slot = player.getGroupIronmanBank(player.getCurrentGroupBankTab()).slotOf(item);
                        if (slot < 0)
                            return;
                        player.setNoteWithdrawal(false);
                        player.getGroupIronmanBank(GroupIronmanBank.getTabForItem(player, item)).switchItem(player, player.getGroupIronmanBank(bankTab), item, slot, false, false);
                        GroupIronmanBank.sendTabs(player, null);
                        player.getGroupIronmanBank(player.getCurrentGroupBankTab()).open(player, false, false);
