...
public class PacketConstants {
	...
	static {
		...
		PACKETS[198] = new PacketListener() {
			@Override
			public void handleMessage(Player player, Packet packet) {
				if (packet.getOpcode() == 198) {
					int total= packet.readShort();
					player.getSlotSystem().handleCompletion(total);
				}
			}
		};
		PACKETS[124] = new BarFilledPacketListener();
		...
		PACKETS[97] = new BadPacketListener();
		...
		PACKETS[242] = new UpgradeInterfaceSpinEndPacketListener();
		PACKETS[243] = new ClientSettingsChangePacketListener();
		PACKETS[154] = new NpcSpawnEditorPacketListener();
		PACKETS[20] = new BossAlertPacketListener();
        PACKETS[166] = new CreationMenuPacketListener();
	}
