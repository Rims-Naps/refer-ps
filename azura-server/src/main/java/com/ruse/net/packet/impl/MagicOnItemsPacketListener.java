...
public class MagicOnItemsPacketListener implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
		if (packet.getOpcode() == MAGIC_ON_GROUNDITEMS) {
			int itemY = packet.readLEShort();
			int itemId = packet.readShort();
			int itemX = packet.readLEShort();
			int spellId = packet.readUnsignedShortA();
			NecromancySpells spell = NecromancySpells.forSpellId(spellId);
			if (spell == null)
				return;
			...
			NecromancySpells magicSpell = NecromancySpells.forSpellId(spellId);
			if (magicSpell == null)
				return;
...
/*if (item.getId() != 22053 && (!item.tradeable() || !item.sellable() || item.getId() == ItemDefinition.COIN_ID)) {
				...
				player.performGraphic(
						new Graphic(magicSpell == NecromancySpells.HIGH_ALCHEMY ? 113 : 112, GraphicHeight.LOW));
			...
			case SUPERHEAT_ITEM:
				for (int i = 0; i < ORE_DATA.length; i++) {
					if (item.getId() == ORE_DATA[i][0]) {
						...
						player.getPacketSender().sendTab(GameSettings.MAGIC_TAB);
						player.getSkillManager().addExperience(Skill.MAGIC, spell.baseExperience());
						player.getSkillManager().addExperience(Skill.JOURNEYMAN, Smelting.getExperience(ORE_DATA[i][4]));
						return;
					}
				}
