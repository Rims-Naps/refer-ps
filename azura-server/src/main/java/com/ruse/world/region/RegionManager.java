...
public final class RegionManager {
    ...
    public static void initialise() {
        ...
        for (int regionId : regionIds) {
            ...
            try {
                ...
                System.out.println("Missing files for region " + regionId + ": "
                        + ids.getLeft() + ".gz and " + ids.getRight() + ".gz");

                createMissingRegionFiles(regionId, ids);
            }
        }
    ...
    private static void createMissingRegionFiles(int regionId, Pair<Integer, Integer> ids) {
        try {
            File outputDir = new File("E:\\Tempest\\maps\\");
            if (!outputDir.exists()) {
                outputDir.mkdirs();
            }

            File landscapeTemplate = new File("E:\\Tempest\\0.gz"); // ids.getLeft()
            File dataTemplate = new File("E:\\Tempest\\1.gz"); // ids.getRight()

            File newLandscape = new File(outputDir, ids.getLeft() + ".gz");
            File newData = new File(outputDir, ids.getRight() + ".gz");

            boolean createdSomething = false;

            if (!newLandscape.exists()) {
                java.nio.file.Files.copy(
                        landscapeTemplate.toPath(),
                        newLandscape.toPath(),
                        java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                System.out.println("Created landscape file for region " + regionId + ": " + newLandscape.getName());
            ...
            if (!newData.exists()) {
                java.nio.file.Files.copy(
                        ...
                        newData.toPath(),
                        java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                System.out.println("Created data file for region " + regionId + ": " + newData.getName());
                createdSomething = true;
            }

            if (!createdSomething) {
                System.out.println("Files already existed for region " + regionId);
            }

        } catch (Exception e) {
            System.err.println("Failed to create missing files for region " + regionId);
            e.printStackTrace();
        }
    }
