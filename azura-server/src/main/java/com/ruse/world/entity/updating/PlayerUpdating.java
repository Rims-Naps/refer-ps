...
public class PlayerUpdating {
	 ...
	 */
	private static final int MAX_NEW_PLAYERS_PER_CYCLE = 80;
	...
	private static void updateChat(PacketBuilder builder, Player target) {
        ...
        builder.put(target.getXpmode().ordinal());
	...
	private static void updateSingleHit(PacketBuilder builder, Player target) {
		...
		builder.putShort(target.getSkillManager().getMaxLevel(Skill.HITPOINTS), ValueType.A);
	}
	...
	private static void updateAppearance(Player player, PacketBuilder out, Player target) {
		...
		Equipment cosmetics = target.getCosmetics();
        ...
        properties.putInt(target.getSpecialPercentage());
		if(target.getTransmog().getCurrentTransformation() != null){
			properties.putShort(-1);
			properties.putShort(target.getTransmog().getCurrentTransformation().getNpcId());
		} else if (player.getNpcTransformationId() <= 0) {
			int[] equip = new int[equipment.capacity()];
			for (int i = 0; i < equipment.capacity(); i++) {
				switch (i) {
					case Equipment.HEAD_SLOT:
						equip[i] = target.isCosmeticHeadOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.CAPE_SLOT:
						equip[i] = target.isCosmeticCapeOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.AMULET_SLOT:
						equip[i] = target.isCosmeticAmuletOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.WEAPON_SLOT:
						equip[i] = target.isCosmeticWeaponOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.BODY_SLOT:
						equip[i] = target.isCosmeticBodyOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.SHIELD_SLOT:
						equip[i] = target.isCosmeticShieldOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.LEG_SLOT:
						equip[i] = target.isCosmeticLegsOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.HANDS_SLOT:
						equip[i] = target.isCosmeticGlovesOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					case Equipment.FEET_SLOT:
						equip[i] = target.isCosmeticBootsOn() ? cosmetics.getItems()[i].getId() : equipment.getItems()[i].getId();
						break;
					default:
			...
			if (appearance.getLook()[Appearance.BEARD] <= 0 || appearance.getGender() == Gender.FEMALE) {
				properties.put(0);
		...
		properties.putInt(target.getSkillManager().getCombatLevel());
		properties.putShort(target.getRights().ordinal());
		properties.putShort(target.getLoyaltyTitle().ordinal());
		Item firstCustom = target.getEquipment().getItems()[Equipment.WINGS];
		Item secondCustom = target.getEquipment().getItems()[Equipment.AURA_SLOT];
		Item thirdcustom = target.getEquipment().getItems()[Equipment.ASCEND];

		properties.putShort(firstCustom == null || firstCustom.getId() <= 0 ? 0 : firstCustom.getId());
		properties.putShort(secondCustom == null || secondCustom.getId() <= 0 ? 0 : secondCustom.getId());
		properties.putShort(thirdcustom == null || thirdcustom.getId() <= 0 ? 0 : thirdcustom.getId());
