...
import com.ruse.world.content.AttributeMap;
import com.ruse.world.content.Attributes;
...
public abstract class Character extends Entity {
	...
	public abstract void burnvictim(Character victim, CombatType type);
	...
	public Hit decrementHealth(Hit hit) {
		...
		if (hit.getDamage() < 0)
			hit.setDamage(0);
		int outcome = (int) (getConstitution() - hit.getDamage());
		if (outcome < 0)
			outcome = 0;
	 ...
	 */
	public void dealTripleDamage(Hit hit, Hit secondHit, Hit thirdHit) {
	 ...
	 */
	public void dealQuadrupleDamage(Hit hit, Hit secondHit, Hit thirdHit, Hit fourthHit) {
		dealDoubleDamage(hit, secondHit);
	...
	public int getAndDecrementBurnDmg() {
		return burndamage -= 1;
	}
	...
	public void delayedMoveTo(Position teleportTarget, int delay) {
		if (moving)
			return;
		moving = true;
	...
	public void performForcedAnimation(Animation animation) {
		setAnimation(animation);
	}
	...
	private final AttributeMap attributeMap = new AttributeMap(this);
