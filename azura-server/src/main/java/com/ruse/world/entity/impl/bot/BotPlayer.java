...
public class BotPlayer extends Player {
     ...
     *
     * @param activeScript The active script.
     */
     ...
     *
     * @param sessionEnd The session's length.
     */
    ...
    public BotPlayer(String username, Position position, PlayerSession session) {
        ...
        if (PlayerSaving.playerExists(username)) {
            System.out
                    .println("Error: unable to add bot '" + username + "' as a player with that name already exists.");
            return;
        }
    ...
    public void randomizeLevels() {
        for (int i = 0; i <= 6; i++) {
            int level = Misc.random(60, 120);

            getSkillManager().setMaxLevel(Skill.forId(i), level);
        }
    }
