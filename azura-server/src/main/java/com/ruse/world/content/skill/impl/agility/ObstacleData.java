...
public enum ObstacleData {
	...
	ROPE_SWING(2282, true) {
		...
		public void cross(Player player) {
			...
			boolean success = Agility.isSucessive(player);
			TaskManager.submit(new Task(1, player, false) {
				int tick = 0;
				...
				public void execute() {
					...
					if (!success) {
						...
						player.dealDamage(new Hit(Misc.getRandom(50), Hitmask.NEUTRAL, CombatIcon.NONE));
	...
	LOG_2(2294, true) {
		@Override
		public void cross(Player player) {
			boolean fail = !Agility.isSucessive(player);
	...
	BALANCE_LEDGE(2302, true) {
		...
		public void cross(Player player) {
			...
			boolean fallDown = !Agility.isSucessive(player);
	...
	RAMP(1948, false) {
		...
		public void cross(Player player) {
			...
			boolean first = player.getPosition().getX() == 2535;
			boolean oneStep = player.getPosition().getX() == 2537 || player.getPosition().getX() == 2542;
				...
				public void execute() {
					...
					if (tick == 2) {
						if (fail) {
							...
							player.dealDamage(new Hit(Misc.getRandom(60), Hitmask.NEUTRAL, CombatIcon.NONE));
							Agility.addExperience(player, 40);
							player.getPacketSender().sendMessage("You failed to swing your way across.");
							stop();
							return;
	...
	RED_DRAGON_LOG_BALANCE(5088, false) {
		...
		public void cross(Player player) {
			...
			player.getUpdateFlag().flag(Flag.APPEARANCE);
			int moveX = player.getPosition().getX() > 2683 ? 2686 : 2683;
