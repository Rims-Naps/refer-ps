...
public class SpectralRaidHardChest {
    private static final int KEY_ID = 2055;

    private static final  int[] TRINKET_MATERIAL = new int[] {431};
    private static final  int[] UPGRADE_MATERIALS = new int[] {2051,2052,3007, 2619};
    private static final  int[] RING_RARE = new int[] {23057,15670};
    private static final  int[] SUPER_RARE = new int[] {418,417, 10256, 10260, 10262, 15668};
    private static final int[] RARE = new int[] {10946,15668,17130};
    private static final int[] UNCOMMON = new int[] {417, 730};
    private static final int[] COMMON = new int[] { 15667, 1446, 20109, 20111,20112, 17582,17586,17584};
    ...
    public static void generateReward(Player player){
        ...
        if (Misc.getRandom(700) == 1) {
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID_HARD, new Item(trinketrare));
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID_HARD, new Item(axerares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID_HARD, new Item(ringRares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID_HARD, new Item(superrares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID_HARD, new Item(rares));

        }
        else if (Misc.inclusiveRandom(12) == 1) {
            int uncommons = getRandomItem(UNCOMMON);
            player.getInventory().addDropIfFull(uncommons, Misc.random(1));
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID_HARD, new Item(uncommons));


        } else {
            int commons = getRandomItem(COMMON);
            player.getInventory().addDropIfFull(commons, 1);
            player.getInventory().addDropIfFull(995, Misc.random(175,625));
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID_HARD, new Item(commons));
