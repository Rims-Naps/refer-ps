...
public class DungeoneeringParty {
    ...
    public static void create(Player p) {
     ...
     //   if (p.getLocation() != Locations.Location.CUSTOM_RAIDS_LOBBY) {
     ...
     //   }
        ...
        p.getPacketSender().sendMessage(
                "<img=5> <col=660000>You have created a Dungeoneering party. Perhaps you should invite a few players?");
        p.getPacketSender().sendDungeoneeringTabIcon(true).sendTabInterface(GameSettings.PLAYER_TAB, 26224)
                .sendTab(GameSettings.PLAYER_TAB);
    ...
    public void invite(Player p) {
        ...
        if (getOwner().getGameMode() == GameMode.GROUP_IRON && !getOwner().getIronmanGroup().equals(p.getIronmanGroup())) {
            p.getPacketSender().sendMessage("You are not a part of this players ironman group.");
            return;
        }
        if (p.getGameMode() == GameMode.GROUP_IRON && !p.getIronmanGroup().equals(getOwner().getIronmanGroup())) {
            p.getPacketSender().sendMessage("You are not a part of this players ironman group.");
            return;
        }
    ...
    public void add(Player p) {
        ...
        p.getPacketSender().sendMessage("You have joined " + getOwner().getUsername() + "'s party.");
