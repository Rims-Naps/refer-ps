...
public class ConvertCoins {
	...
	public static void convertMillCoins(Player player) {
		...
		if (player.getInventory().getAmount(ItemDefinition.COIN_ID) >= 1_000) {
			...
			player.sendMessage("You have successfully converted coins to Athens tokens");
		} else {
			player.sendMessage("You don't have enough coins to convert to Athens tokens");
		}
	}
	/*
	public static void convertMillCoins(Player player) {
		if (player.getInventory().getAmount(ItemDefinition.COIN_ID) >= 1_000_000) {
			...
			player.sendMessage("You have successfully converted your Athens tokens to 1m bag");
		} else {
