...
public class SpectralChest {

    private static final int KEY_ID = 2062;
    private static final  int[] SUPER_RARE = new int[] {2043, 2044, 2045, 2046,2047};
    private static final int[] RARE = new int[] {457, 458, 298,10946, 10262, 10260, 10256,2061,995,};
    private static final int[] UNCOMMON = new int[] {10945, 995,401,402,995,403,404,405};
    private static final int[] COMMON = new int[] {995, 1302, 995, 1303, 1304, 358, 995, 19062, 3576, 995, 6466, 2064, 1448};
    ...
    public static void generateReward(Player player){
        ...
        if (Misc.getRandom(500) == 1) {
            ...
            World.sendMessage("@red@<shad=0>[SPECTRAL]<shad=0>@red@ " + player.getUsername() + " @red@<shad=0>pulled a <shad=0>@red@" +  ItemDefinition.forId(superrares).getName() + " @red@<shad=0>from the Spectral Chest!");
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_KEY, new Item(superrares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
        else if (Misc.getRandom(220) == 1) {
            int rares = getRandomItem(RARE);
            player.getInventory().addDropIfFull(rares, 1);
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_KEY, new Item(rares));

        }
        else if (Misc.getRandom(100) == 1) {
            int uncommons = getRandomItem(UNCOMMON);
            player.getInventory().addDropIfFull(uncommons, 1);
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_KEY, new Item(uncommons));


        } else {

            int commons = getRandomItem(COMMON);
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_KEY, new Item(commons));
            ...
            } else if (ItemDefinition.forId(commons).getId() == 2064){
                int amount_spectral_essence = Misc.random(5,15);
                player.getInventory().addDropIfFull(commons, amount_spectral_essence);
                player.sendMessage("rolled " + amount_spectral_essence + " Spectral Essence from chest");
