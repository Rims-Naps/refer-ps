...
public class DefaultRangedCombatStrategy implements CombatStrategy {
	...
	public CombatContainer attack(Character entity, Character victim) {
		if (entity.isNpc()) {
			NPC npc = (NPC) entity;
			AmmunitionData ammo = AmmunitionData.BRONZE_ARROW;
			switch (npc.getId()) {
			case 688:
		...
		Player player = (Player) entity;
		boolean dBow = CombatFactory.darkBow(player);
	...
	public static void fireProjectile(Character e, Character victim, AmmunitionData ammo, boolean dBow) {
	...
	public int attackDistance(Character entity) {
		...
		if (player.getPosition().getRegionId() == 10542){
			distance += 12;
		}
		if (player.getPosition().getRegionId() == 11594){
			distance += 2;
		}
		if (player.getPosition().getRegionId() == 11059){
			distance += 12;
		}
		if (player.getPosition().getRegionId() == 9009){
			distance += 1;
		}

		if (player.getEquipment().contains(1462)) {
			distance += 4;
		}
	...
	private boolean checkAmmo(Player player) {
		...
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 22010
				...
				|| player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 711
				|| player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 712
				|| player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 713
				|| player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 714
				...
				|| player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 10230
	...
	public static void decrementAmmo(Player player, Position pos) {
		...
		boolean ardy = player.getEquipment().get(Equipment.CAPE_SLOT).getId() == 19748;
		...
		boolean skillcape = player.getSkillManager().skillCape(Skill.RANGED);
        ...
        if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 1462)
            return;

		// Decrement the ammo in the selected slot.
		if (Misc.random(0,10) > 5) {
	...
	private static int getModifiedDamage(Player player, Character target, CombatContainer container) {
		if (container == null || container.getHits().length < 1)
			return 0;
		ContainerHit hit = container.getHits()[0];
		if (!hit.isAccurate())
			return 0;
		int damage = (int) container.getHits()[0].getHit().getDamage();
		...
		case 9241: // Magical Posion
			...
			CombatFactory.burnentity(target, BurnType.BASE_PRAYER);
			break;
		case 9242:
			if (player.getSkillManager().getCurrentLevel(Skill.HITPOINTS)
					- player.getSkillManager().getCurrentLevel(Skill.HITPOINTS) / 200 < 10) {
				break;
			}
			int priceDamage = (int) (player.getSkillManager().getCurrentLevel(Skill.HITPOINTS) * 0.08);
			if (priceDamage < 0) {
				return damage;
			}
			...
			player.dealDamage(new Hit(priceDamage, Hitmask.NEUTRAL, CombatIcon.RANGED));
			return dmg2;
		case 9243:
