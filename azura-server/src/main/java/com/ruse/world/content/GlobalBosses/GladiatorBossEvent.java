...
public class GladiatorBossEvent {

    public static NPC currentBoss = null;
    public static boolean isAlive = false;

    public static int tick = -12_000;

    public static Position SPAWN_POINT = new Position(2783, 3540,0);
    ...
    public static void sequence() {
        ...
        if (!isAlive && tick % 12_000 == 11_970) {
            String message = "Gladiator will spawn in 30 seconds!";
            World.sendMessage(message);
        }
        if (!isAlive && tick % 12_000 == 11_940) {
            String message = "Gladiator will spawn in 1 minute!";
            World.sendMessage(message);
        }

        if (!isAlive && tick % 12_000 == 11_500) {
            String message = "Gladiator will spawn in 5 minutes!";
            World.sendMessage(message);
        }
        ...
        if (tick % 12_000 == 0) {
            if(currentBoss == null || currentBoss.isDying() || !currentBoss.isRegistered() && !isAlive) {
                ...
                if (boss == 559) {
                    message = "Gladiator has spawned!";
                    message2 = "<shad=0>@red@[GLADIATOR] Gladiator has landed!! ::glad";
                }
