...
public class SmithingData {
	...
	public static boolean canSmelt(Player player, int barId) {
		if (getLevelReq(barId) > player.getSkillManager().getCurrentLevel(Skill.JOURNEYMAN)) {
	...
	private static String GetForlvl(int i, Player player) {
		if (player.getSkillManager().getMaxLevel(Skill.JOURNEYMAN) >= i)
			return "@whi@";
	...
	public static boolean ironOreSuccess(Player player) {
		return Misc.getRandom((int) (1 + player.getSkillManager().getCurrentLevel(Skill.JOURNEYMAN) / 0.5)) > 5;
	}
}
