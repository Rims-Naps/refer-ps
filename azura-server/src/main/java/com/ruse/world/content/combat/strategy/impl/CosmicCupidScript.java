...
public class CosmicCupidScript implements CombatStrategy {
    ...
    public boolean customContainerAttack(Character entity, Character victim) {
        ...
        for(Player otherVictims : World.getNearbyPlayers(boss.getPosition(), 25)) {
            new Projectile(boss, otherVictims, 1702, 75, 10, 110, 0, 0).sendProjectile();
            ...
            TaskManager.submit(new Task(3, otherVictims, false) {
                ...
                public void execute() {
                    ...
                    if (otherVictims.getCurseActive()[CurseHandler.BLOCK_MELEE]) {
                        otherVictims.dealDamage(new Hit(Misc.random(0), Hitmask.FIRE, CombatIcon.BLOCK));
                    }
    ...
    private void sendStage(NPC boss, int execStage) {
        ...
        switch (execStage) {
            case 0:
                GameSettings.cupidBossStage = 2;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(1639));
                }
                break;
            case 1:
                GameSettings.cupidBossStage = 1;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(1619));
                }
                break;
            case 2:
                GameSettings.cupidBossStage = 0;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(1702));
                }
                break;
        }
    }
