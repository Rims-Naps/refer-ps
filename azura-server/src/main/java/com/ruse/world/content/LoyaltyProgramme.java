...
public class LoyaltyProgramme {
					...
					if (p.getPointsHandler().getBarrowsChests() < 10) {
						if (sendMessage)
							p.getPacketSender()
									...
									.sendMessage("You have currently looted the chest "
	...
	public static void unlock(Player player, LoyaltyTitles title) {
		...
		player.getPacketSender()
				.sendMessage("You have unlocked the " + Misc.formatText(title.name().toLowerCase()) + " loyalty title!");
	}
		...
		if (title != null) {
			if (title.canBuy(player, true)) {
				...
				if (player.getPointsHandler().getLoyaltyPoints() >= title.cost) {
					...
					player.getPacketSender().sendMessage("You have changed your title.");
