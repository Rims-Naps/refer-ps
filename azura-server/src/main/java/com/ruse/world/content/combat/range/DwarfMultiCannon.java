...
public class DwarfMultiCannon {
	...
	public static void setupCannon(Player c) {
		...
		boolean movementLock = c.getMovementQueue().isLockMovement();
		c.getMovementQueue().setLockMovement(true);
		GameObject object = new GameObject(CANNON_BASE, c.getPosition().copy());
		GameObject object2 = new GameObject(CANNON_STAND, c.getPosition().copy());
		GameObject object3 = new GameObject(CANNON_BARRELS, c.getPosition().copy());
		GameObject object4 = new GameObject(CANNON, c.getPosition().copy());
	...
	public static void fireCannon(Player c, DwarfCannon cannon) {
		if (cannon.cannonFiring())
			return;
	...
	private static void rotateCannon(Player c, DwarfCannon cannon) {
		GameObject object = c.getCannon().getObject();
	...
	public static void attack(Player player) {
		...
		if (n == null)
			return;
		Hit dmg = new Hit(Misc.getRandom(266) - Misc.getRandom(n.getDefinition().getDefenceRange()), Hitmask.NEUTRAL,
				CombatIcon.RANGED);
		...
		n.getCombatBuilder().addDamage((Character) player, (int) dmg.getDamage());
		player.getSkillManager().addExperience(Skill.RANGED, (int) (((dmg.getDamage() * .50))));
		player.getSkillManager().addExperience(Skill.HITPOINTS, (int) ((dmg.getDamage() * .30)));
