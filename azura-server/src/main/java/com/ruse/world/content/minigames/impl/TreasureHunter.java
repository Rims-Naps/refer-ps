...
public class TreasureHunter {
    ...
    public static int NPC_1 = 13459, NPC_2 = -1, NPC_3 = 9817, NPC_4 = 9818;
    ...
    public static void addKey(Player player, NPC npc) {
       /* int chanceInt = Misc.getRandom(3);
        if (chanceInt == 1) {
            ...
            System.out.println("Congratulations, You have received a key from killing a " + npc.getDefinition().getName() + "!");
        }*/
    }
        ...
        if (used.getId() >= KEY_1 && used.getId() <= KEY_4 && usedWith.getId() >= KEY_1 && usedWith.getId() <= KEY_4) {
            if (player.getInventory().contains(new Item[]{new Item(KEY_1), new Item(KEY_2), new Item(KEY_3), new Item(KEY_4)})) {
                ...
                if (Misc.getRandom(100) <= 75){
                    ...
                    player.sendMessage("You have successfully created the Master Key.");
                }else{
                    player.sendMessage("You have failed the Master Key creation.");
                }
                return true;
            }
        }
