...
public class PouchMaking {
	...
	public static boolean pouchInterface(Player p, int buttonId) {
		Pouch pouch = Pouch.get(buttonId);
		if (pouch == null)
			return false;
	...
	private static boolean hasRequirements(Player player, Pouch pouch) {
		if (pouch == null)
			return false;
		player.getPacketSender().sendClientRightClickRemoval();
		if (player.getSkillManager().getMaxLevel(Skill.NECROMANCY) >= pouch.getLevelRequired()) {
	...
	public static void infusePouches(Player player, int amount) {
		Pouch pouch = player.getSelectedPouch();
		if (pouch == null)
			return;
			...
			public void execute() {
				...
				while (x > 0) {
					if (!hasRequirements(player, pouch))
						...
						if (player.getSkillManager().skillCape(Skill.NECROMANCY)) {
						...
						player.getSkillManager().addExperience(Skill.NECROMANCY, pouch.getExp());
