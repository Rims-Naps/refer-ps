...
import static com.ruse.model.Skill.MAGIC;
...
public class Consumables {
    ...
    private static void eat(Player player, FoodType food, int slot) {
        ...
        if (food != null && player.getFoodTimer().elapsed(1100)) {
            ...
            if (food == FoodType.ROCKTAIL) {
                int max = (player.getSkillManager().getMaxLevel(Skill.HITPOINTS) + 100);
                if (nexEffect)
                    max = (player.getSkillManager().getMaxLevel(Skill.HITPOINTS) + 400);
                if (player.getSkillManager().getCurrentLevel(Skill.HITPOINTS) >= (max)) {
                    heal = 100;
                }
            ...
            player.setConstitution((int) (player.getConstitution() + heal));
    ...
    public static boolean isPotion(int itemId) {
        ...
        return pot.equalsIgnoreCase("brandy");
    }
    ...
    public static boolean drinkInfiniteOverload(Player player, int time) {
        return false;
    }

    public static void drinkInfinitePrayerPotion(Player player, int amount) {
            player.getSkillManager().setCurrentLevel(Skill.PRAYER, (int) (player.getSkillManager().getCurrentLevel(Skill.PRAYER) + amount));
    ...
    public static void drinkHealthPotion(Player player, int amount) {
        player.heal(amount);
        player.performAnimation(new Animation(829));
        //player.sendMessage("@gre@Some of your health is restored");
    }
    ...
    /*            case 23124:
                    if(!drinkOverload(player, slot, 23124))
                        return;
                    break;
                case 23125:
                    if(!drinkOverload(player, slot, 23125))
                        return;
                    break;
                case 23126:
                    if(!drinkOverload(player, slot, 23126))
                        return;
                    break;
                case 15328:
                    if(!drinkOverload(player, slot, 15328))
                        return;
                    break;
                case 2831:
                    if(!drinkOverload(player, slot, 2831))
                        return;
                    break;*/
                //Restore potions
                case 23121: //T1
                    ...
                    break;
                case 23122: //T1
                    drinkInfinitePrayerPotion(player,400);
                    break;
                case 23123: //T1
                    drinkInfinitePrayerPotion(player,600);
                    break;
                case 23118:
                    drinkHealthPotion(player, 200);
                    break;
                case 23119:
                    drinkHealthPotion(player, 400);
                    break;
                case 23120:
                    drinkHealthPotion(player, 600);
                    break;
             ...
             /*   case 23124: //T1
                ...
                case 15328: //T3
                    if (!drinkInfiniteOverload(player, 600))
                        return;
                    break;

                case 2831: //T3
                    if (!drinkInfiniteOverload(player, 1000))
                        return;
                    break;*/
/*
                     * Attack potions
                ...
                case 12140: // Summoning pot 4
                    ...
                    player.getSkillManager().setCurrentLevel(Skill.NECROMANCY,
                            (int) (player.getSkillManager().getCurrentLevel(Skill.NECROMANCY)
                                    + (player.getSkillManager().getMaxLevel(Skill.NECROMANCY) * 0.25)));
                    if (player.getSkillManager().getCurrentLevel(Skill.NECROMANCY) > player.getSkillManager()
                            .getMaxLevel(Skill.NECROMANCY))
                        player.getSkillManager().setCurrentLevel(Skill.NECROMANCY,
                ...
                case 6691:
                    ...
                    break;
                case 15312:
                    if (player.getLocation() != null && player.getLocation() == Location.GAMBLE) {
                        player.getPacketSender().sendMessage("You cannot use this potion in the GAMBLE.");
                        return;
                    }
                    player.performAnimation(new Animation(829));
    ...
    public static void drinkStatPotion(Player player, int potion, int replacePotion, int slot, int skill,
                                       boolean super_pot) {
        if (slot >= 0) {
            player.performAnimation(new Animation(829));
    ...
    public static boolean drinkOverload(Player player, int slot, int replacePotion) {
        return false;
    }

    public static boolean drinkSuperOverload(Player player, int slot, int replacePotion) {
        return false;
    }

    public static boolean drinkRestore(Player player, int slot, int replacePotion) {
        if (player.getLocation() == Location.GAMBLE || player.getLocation() == Location.DONORISLE) {
    ...
    public static void overloadIncrease(Player player, Skill skill, double l) {
    ...
    public static void levelIncrease(Player player, Skill skill, int level) {
        player.getSkillManager().setCurrentLevel(skill, level, true);
    }

    public static void resetCombatBoosts(Player player) {
        player.getSkillManager().setCurrentLevel(ATTACK, player.getSkillManager().getMaxLevel(ATTACK), true);
        player.getSkillManager().setCurrentLevel(STRENGTH, player.getSkillManager().getMaxLevel(STRENGTH), true);
        player.getSkillManager().setCurrentLevel(DEFENCE, player.getSkillManager().getMaxLevel(DEFENCE), true);
        player.getSkillManager().setCurrentLevel(RANGED, player.getSkillManager().getMaxLevel(RANGED), true);
        player.getSkillManager().setCurrentLevel(MAGIC, player.getSkillManager().getMaxLevel(MAGIC), true);
    }
