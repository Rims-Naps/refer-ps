...
public class TowerSuperior implements CombatStrategy {
    ...
    private void groundAttack(Player player, NPC boss) {
        ...
        boss.forceChat("YOU DARE CHALLENGE ME!!!");
    ...
    private void prayerAttack(Player player, NPC boss) {
        ...
        if (prayeroff == 1) {
            ...
            player.sendMessage("<shad=0>@red@Tower Superior disabled your prayers..");
        }
        ...
        TaskManager.submit(new Task(5, player, false) {
            ...
            public void execute() {
                ...
                if (doubleDamage)
                    modifier = 2;
                if (!player.is_dead_in_tower()) {
                    if (player.getCurseActive()[CurseHandler.AQUASTRIKE]) {
                        player.dealDamage(new Hit(0, Hitmask.NONE, CombatIcon.BLOCK));
                        player.sendMessage("<shad=0>@gre@You successfully blocked Tower Superior's Water attack");
                    } else {
                        player.dealDamage(new Hit(35 * modifier, Hitmask.CRITICAL, CombatIcon.RANGED));
                        player.sendMessage("<shad=0>@red@You failed to block Tower Superior's Water attack");
                        player.msgRed("Use Aquastrike to avoid massive damage!");
