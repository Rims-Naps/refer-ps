...
public class GeomancerScript implements CombatStrategy {
    ...
    private void specialAttack(Player player, NPC boss) {
        if (specialRunning || rangeRunning)
            return;

        specialRunning = true;
        Position playerSpot = player.getPosition();
        boss.forceChat("ENTANGLED!");
        int prayeroff = Misc.random(1, 50);
        if (prayeroff == 1) {
            PrayerHandler.deactivateAll(player);
            CurseHandler.deactivateAll(player);
            player.msgRed("Geomancer disabled your prayers!");
        }
        new Projectile(boss, player, 2720, 100, 5, 65, 35, 0).sendProjectile();
        boss.performGraphic(new Graphic(184));
        ...
        TaskManager.submit(new Task(2, player, false) {
            ...
            public void execute() {
                if (player.getCurseActive()[CurseHandler.BLOCK_RANGE]) {
                    ...
                    player.dealDamage(new Hit(Misc.random(45,120), Hitmask.CRITICAL, CombatIcon.RANGED));
                }
                rangeRunning = false;
                this.stop();
