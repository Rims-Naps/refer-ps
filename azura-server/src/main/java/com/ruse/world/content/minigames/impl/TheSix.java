...
public class TheSix {
	...
	public void enter(boolean clan) {
		...
		Position pos = new Position(2384, 4721, z);
		Position orig = player.getPosition().copy();
	...
	public void leave(boolean move) {
		...
		int killcount = player.getMinigameAttributes().getBarrowsMinigameAttributes().getKillcount();
		if (killcount > 0) {
			int points = player.doingClanBarrows() ? 1 * killcount : 2 * killcount;
			player.getPointsHandler().setBarrowsPoints(points, true);
			player.getPacketSender().sendMessage("You have received " + points + " Barrows points.");
		}
