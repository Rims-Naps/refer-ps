package com.ruse.world.content.new_raids_system.raids_party;
...
public class RaidsParty {
    ...
    public void invite(Player p) {
        ...
        if (hasEnteredDungeon) {
            getOwner().getPacketSender().sendMessage("@red@You cannot attempt to Invite anyone right now.");
            return;
        }
        if (player_members.size() >= 5) {
            getOwner().getPacketSender().sendMessage("@red@Your party is full.");
            return;
        }

        if (player_members.contains(p)) {
            getOwner().getPacketSender().sendMessage("@blu@That player is already in your party.");
            return;
        }
        if (p.getMinigameAttributes().getRaidsAttributes().getParty() != null) {
            getOwner().getPacketSender().sendMessage("@red@That player is currently in another party.");
            return;
        }
        if (p.getRights() != PlayerRights.DEVELOPER && System.currentTimeMillis()
            ...
            getOwner().getPacketSender().sendMessage("@red@You must wait 2 seconds between each party invitation.");
            return;
        }
        if (p.busy()) {
            getOwner().getPacketSender().sendMessage("@red@That player is currently busy.");
            return;
        }
        if (p.getLocation() != Locations.Location.CORRUPT_RAID_LOBBY) {
            return;
        } else if ((p.getLocation() == Locations.Location.CORRUPT_RAID_LOBBY && getOwner().getLocation() == Locations.Location.CORRUPT_RAID_LOBBY)){
            ...
            getOwner().getPacketSender().sendMessage("@blu@An invitation has been sent to " + p.getUsername() + ".");
        }
    }
    ...
    public void add(Player p) {
		...
		/*if (p.getLocation() != Locations.Location.VENGEANCE_ENTRANCE || p.getLocation() != Locations.Location.RAIDS_TWO_ENTRANCE || p.isTeleporting()) {
			return;
		}*/
    ...
    public void remove(Player p, boolean reset, boolean fromParty) {
        ...
        p.getPacketSender().sendString(58009, "@gre@Create");
        int id = 58016;
        ...
        p.getPacketSender().sendString(58002, "@yel@Raid Party: @whi@0");

        if (p == owner) {
            if (p.getLocation() == Locations.Location.CORRUPT_RAID_ROOM_1) {
                p.moveTo(new Position(2974, 3879, 0));
            }

            if (player_members.size() > 0) { // if there are still members in the party
                // select a new leader
                owner = player_members.get(0);
                owner.getPacketSender().sendMessage("You are now the leader of the party.");
                sendMessage(owner.getUsername() + " is the new leader of the party.");
            }
            for (Player member : player_members) {
                if (member != null && member.getMinigameAttributes().getRaidsAttributes().getParty() != null
                    ...
                    if (fromParty) {
                        ...
                        member.forceLeaveRaid(false);
                    } else {//i did that because it leave him in raid without party
            ...
            if (reset) {
                p.getPacketSender().sendTabInterface(GameSettings.ACHIEVEMENT_TAB, 35600);
                p.getPacketSender().sendTab(GameSettings.ACHIEVEMENT_TAB);
            }
        }
        if (fromParty) {
    ...
    public void refreshInterface() {
        for (Player member : getPlayers()) {
            if (member != null) {
                // Check if the member is the party leader
                if (member.equals(owner)) {
                    member.getPacketSender().sendString(58009, "@gre@Invite"); // Change color to yellow
                } else {
                    member.getPacketSender().sendString(58009, "@red@Invite"); // Default color for other party members
                }
                ...
                for (int i = 0; i < getPlayers().size(); i++) {
                    ...
                    // Check if the member is the party leader
                    if (getPlayers().get(i).equals(owner)) {
                        member.getPacketSender().sendString(start++, "@yel@" + getPlayers().get(i).getUsername()); // Change color to yellow
                    } else {
                        member.getPacketSender().sendString(start++, "@whi@" + getPlayers().get(i).getUsername()); // Default color for other party members
                    }
                    if (member.getLocation() == Locations.Location.CORRUPT_RAID_LOBBY) {
                        member.getPacketSender().sendString(start++, "                             @gre@E:@whi@" + getPlayers().get(i).getCorruptCompletions() + " @yel@M:@whi@" + getPlayers().get(i).getCorruptMediumCompletions() + " @red@H:@whi@" + getPlayers().get(i).getCorruptHardCompletions());
                    }
                    if (member.getLocation() == Locations.Location.VOID_RAID_LOBBY) {
                        member.getPacketSender().sendString(start++, "                             @gre@E:@whi@" + getPlayers().get(i).getVoidCompletions() + " @yel@M:@whi@" + getPlayers().get(i).getVoidMediumCompletions() + " @red@H:@whi@" + getPlayers().get(i).getVoidHardCompletions());
                    }


                }

                for (int i = start; i < 58064; i++) {
                    start++;
                ...
                member.getPacketSender().sendString(58002, "@whi@Raid Party: @whi@" + getPlayers().size());
            }
        }
    }
    ...
    public int getPlayersInRaidsLocation(RaidsParty party)
    {
        ...
        {
            if (member.getLocation().equals(Locations.Location.CORRUPT_RAID_ROOM_1)
                    || member.getLocation().equals(Locations.Location.CORRUPT_RAID_ROOM_2)
                    || member.getLocation().equals(Locations.Location.CORRUPT_RAID_ROOM_3)
                    || member.getLocation().equals(Locations.Location.VOID_RAID_ROOM_1)
                    || member.getLocation().equals(Locations.Location.VOID_RAID_ROOM_2)
                    || member.getLocation().equals(Locations.Location.VOID_RAID_ROOM_3))
            {
                inRaids++;
            }
        }
