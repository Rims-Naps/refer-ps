...
public class Dungeoneering {
	...
	public static void start(Player p) {
			...
			p.sendMessage("You haven't created a raid party yet, or aren't in one"); // dungewon
			return;
			}
	...
	public static void leave(Player p, boolean resetTab, boolean leaveParty) {
		if (p.getMinigameAttributes().getDungeoneeringAttributes().getParty() != null) {
			...
			p.getPacketSender().sendTabInterface(GameSettings.PLAYER_TAB, 111000);//
			p.getPacketSender().sendDungeoneeringTabIcon(false);
			p.getPacketSender().sendTab(GameSettings.PLAYER_TAB);
		}
	}
	...
	public static void setupFloor(DungeoneeringParty party) {
		 ...
		 * Spawning npcs
		 */
		NPC smuggler = new NPC(55, new Position(party.getDungeoneeringFloor().getSmugglerPosition().getX(),
	...
	public static final int DUNGEONEERING_GATESTONE_ID = 9999;
}
