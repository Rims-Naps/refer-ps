...
public class TeleportHandler2 {
	...
	public static void teleportPlayer(Player player, Position targetLocation, TeleportType teleportType) {
				...
				default:
					if(tick == teleportType.getStartTick()) {
						...
						if(Dungeoneering.doingOldDungeoneering(player)) {
							Position dungEntrance = player.getMinigameAttributes().getDungeoneeringAttributes().getParty().getDungeoneeringFloor().getEntrance().copy().setZ(player.getPosition().getZ());
	...
	public static void teleportPlayer2(Player player, Position targetLocation, TeleportType teleportType) {
		if(teleportType != TeleportType.LEVER) {
			if(!checkReqs(player, targetLocation)) {
				return;
			}
