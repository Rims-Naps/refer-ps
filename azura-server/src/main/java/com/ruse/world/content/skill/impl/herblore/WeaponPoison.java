...
public class WeaponPoison {
	 ...
	 */
	public static void execute(Player player, int itemUse, int useWith) {
		Weapon weapon = Weapon.weapon.get(useWith);
		if (weapon != null) {
			...
			if (w != null) {
				...
				if (random > 0) {
					if (Misc.getRandom(random) == 1)
						CombatFactory.burnentity(target, random == 5 ? BurnType.TIER2_PRAYER : BurnType.BASE_PRAYER);
					break;
				}
			}
		}
