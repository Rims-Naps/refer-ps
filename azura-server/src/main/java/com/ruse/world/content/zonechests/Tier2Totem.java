...
public class Tier2Totem {
    private static final int TIER_2_TOTEM_ID = 20081;

    public static final  int[] CAPE_RARE = new int[] {15713, 15710, 15715};

    public static final  int[] SUPER_RARE = new int[] { 20007, 20008};
    ...
    public static void handleChest(Player p) {
        if (!p.getClickDelay().elapsed(3000))
            return;
        if (!p.getInventory().contains(TIER_2_TOTEM_ID)) { //key id
        ...
        p.getInventory().delete(TIER_2_TOTEM_ID, 1);
        ...
        p.getBoxTracker().add(new BoxesTracker.BoxEntry(TIER_2_TOTEM_ID, 1));
        p.sendMessage("You have now opened " + BoxesTracker.getTotalBoxesOpenedForItem(TIER_2_TOTEM_ID, p) + " " + ItemDefinition.forId(TIER_2_TOTEM_ID).getName());
        if (p.getMediumTasks().hasCompletedAll() && !p.getEliteTasks().hasCompletedAll()) {
            EliteTasks.doProgress(p, EliteTasks.EliteTaskData.OFFER_5x_T2_TOTEM, 1);
        }
        generateReward(p);
    }
    ...
    public static void generateReward(Player player){
        ...
        if (Misc.random(player_save_chance) == 1 && player.getMembershipTier().isMember()) {
            ...
            player.getInventory().addDropIfFull(TIER_2_TOTEM_ID, 1);
        }
        ...
        if (Misc.getRandom(75) == 1) {
            ...
            player.getInventory().addDropIfFull(COMMON, 500);
