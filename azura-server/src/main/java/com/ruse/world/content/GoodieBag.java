...
public class GoodieBag {
    ...
    public static final int[] RARES = {17122, 17123 , 19944, 15792, 15793, 15794, 15795, 15796};

    private int AMULET = 17122;
    private int RING = 17123;
    ...
    public void claim() {
        if (!player.getInventory().contains(3578)){
            return;
        }
        ...
        if (!claimed) {
            if (player.getInventory().contains(boxId)) {
                int reward = ItemDefinition.forId(rewards[player.selectedGoodieBag]).getId();
                showRewards();
                ...
                Item itemCollected = new Item(rewards[player.selectedGoodieBag], 1);
                ...
                player.sendMessage("<col=AF70C3><shad=0>[Hyperion] You received a @red@<shad=0>" + ItemDefinition.forId(rewards[player.selectedGoodieBag]).getName() + " <col=AF70C3><shad=0>from the Goodiebag");

                if (reward == AMULET || reward == RING || reward == CAPE || reward == HAT || reward == BODY || reward == LEGS || reward == GLOVES || reward == BOOTS){
                    World.sendMessage("<col=AF70C3><shad=0>[Hyperion] @red@<shad=0>" + player.getUsername() + "<col=AF70C3><shad=0> pulled @red@<shad=0>" + ItemDefinition.forId(rewards[player.selectedGoodieBag]).getName() + " <col=AF70C3><shad=0>from an Hyperion Goodiebag");
                }
                claimed = true;
                boxId = -1;
            } else {
