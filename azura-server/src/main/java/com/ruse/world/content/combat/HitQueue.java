...
public class HitQueue {
	...
	public final CopyOnWriteArrayList<CombatHit> combat_hits = new CopyOnWriteArrayList<>();
	...
	public static class CombatHit {
		...
		public void handleAttack() {
			...
			if (container.getHits().length != 0 && container.getCombatType() != CombatType.MAGIC || container.isAccurate()) {
				...
				if (attacker.isPlayer()) {
					...
					if (damage > 0) {
						if ( p.getLocation() == Location.NECROMANCY_GAME_AREA) {
						...
						if (aoeData != null && aoeData.getRadius() > 0) {
							AOEHandler.handleAttack(p, victim, aoeData.getMinDamage(), aoeData.getMaxDamage(), aoeData.getRadius() + p.getAoeToken().getAoe_size(), aoeData.getIcon());
						}
					}
			...
			if (!container.isAccurate()) {
				...
				CombatFactory.handlePoseidonAbility(attacker,victim,damage,container.getCombatType());

				attacker.burnvictim(victim, container.getCombatType());

				// Finish the magic spell with the correct end graphic.
				if (attacker.isPlayer() && AutoCastSpell.getAutoCastSpell((Player) attacker) != null && attacker.getCurrentlyCasting() != null) {
			...
			if (attacker.isNpc() && victim.isPlayer()) {
                ...
                if (player.getLocation() == Location.TOWER_GAME) {
                    player.setRaidPoints(player.getRaidPoints() + 1);
                }
		...
		public boolean shouldRetaliate() {
				...
				return npc.getMovementCoordinator().getCoordinateState() == CoordinateState.HOME
						&& npc.getLocation() != Location.DUEL_ARENA;
			}
			return false;
