...
public class StaffList {
	...
	public static String getPrefix(Player player) {
		if (player.getRights() == PlayerRights.DEVELOPER) { // 679 main img 0
			return "<img=7>Developer ";
		}
		if (player.getRights() == PlayerRights.OWNER) { // 679 main img 0
			return "<img=4>Owner ";
		}
		if (player.getRights() == PlayerRights.YOUTUBER) { // 679 main img 0
			return "<img=10>Youtuber ";
		}
	...
	public static void delete(Player player) {
        ...
        if (staff.contains("<img=6>" + "@gre@" + player.getUsername()))
            staff.remove("<img=6>" + "@gre@" + player.getUsername());

        if (staff.contains("<img=3>" + "@gre@" + player.getUsername()))
            staff.remove("<img=3>" + "@gre@" + player.getUsername());

        if (staff.contains("<img=7>" + "@gre@" + player.getUsername()))
            staff.remove("<img=7>" + "@gre@" + player.getUsername());

        if (staff.contains("<img=8>" + "@gre@" + player.getUsername()))
            staff.remove("<img=8>" + "@gre@" + player.getUsername());

        if (staff.contains("<img=9>" + "@gre@" + player.getUsername()))
            staff.remove("<img=9>" + "@gre@" + player.getUsername());

        if (staff.contains("<img=10>" + "@gre@" + player.getUsername()))
            staff.remove("<img=10>" + "@gre@" + player.getUsername());
