...
public class IronmanGroup {
    ...
    private GroupIronmanBank[] bank = new GroupIronmanBank[9];
    @Getter
    @Setter
    ...
    public GroupIronmanBank[] getBanks() {return bank;}

    public void setBank(GroupIronmanBank[] bank) {
        this.bank = bank;
    }

    public GroupIronmanBank getBank(int index) {
        return bank[index];
    }

    public IronmanGroup setBank(int index, GroupIronmanBank bank) {
    ...
    public void invite(Player p) {
        if (getOwner() == null || p == getOwner())
            return;
        if (p.getGameMode() != GameMode.GROUP_IRON){
