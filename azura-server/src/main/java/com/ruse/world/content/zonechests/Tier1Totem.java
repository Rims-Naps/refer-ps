...
public class Tier1Totem {
    private static final int TIER_1_TOTEM_ID = 20080;

    public static final  int[] CAPE_RARE = new int[] {15713, 15710, 15715};

    public static final  int[] SUPER_RARE = new int[] { 20006, 20007};
    ...
    public static void handleChest(Player p) {
        if (!p.getClickDelay().elapsed(3000))
            return;
        if (!p.getInventory().contains(TIER_1_TOTEM_ID)) { //key id
        ...
        p.getInventory().delete(TIER_1_TOTEM_ID, 1);
        p.getPacketSender().sendMessage("<shad=0><col=AF70C3>You sacrifice your totem.");
        p.setTotalKeysOpened(p.getTotalKeysOpened() + 1);
        p.getSkillManager().addExperience(Skill.NECROMANCY, 1000);
        p.getDailyTaskInterface().MiscTasksCompleted(0, 1);
        p.getBoxTracker().add(new BoxesTracker.BoxEntry(TIER_1_TOTEM_ID, 1));
        p.sendMessage("You have now opened " + BoxesTracker.getTotalBoxesOpenedForItem(TIER_1_TOTEM_ID, p) + " " + ItemDefinition.forId(TIER_1_TOTEM_ID).getName());
        if (p.getStarterTasks().hasCompletedAll() && !p.getMediumTasks().hasCompletedAll()) {
            MediumTasks.doProgress(p, MediumTasks.MediumTaskData.OFFER_20_T1_TOTEMS, 1);
        }
        generateReward(p);
    }
    ...
    public static void generateReward(Player player){
        ...
        if (Misc.random(player_save_chance) == 1 && player.getMembershipTier().isMember()) {
            ...
            player.getInventory().addDropIfFull(TIER_1_TOTEM_ID, 1);
        }
        ...
        if (Misc.getRandom(100) == 1) {
            ...
            player.getInventory().addDropIfFull(COMMON, 50);
