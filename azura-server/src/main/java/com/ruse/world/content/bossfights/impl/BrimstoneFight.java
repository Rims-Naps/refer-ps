...
public class BrimstoneFight extends BossFight {

    private static final long HEALTH = 75000;

    public BrimstoneFight(Player player) {
        super(player);
    }

    @Override
    public int npcId() {
        return 2017;//SET NPC ID
    }
    ...
    public void begin() {
        if (getPlayer().getSlayer().getSlayerTask().getNpcId() != npcId()) {
            endFight();
            getPlayer().sendMessage("ENDING DUE TO NON MATCH TASK NPC ID");
            return;
        }
        setGlobalHeight(getGlobalHeight() + 4);
        setMyHeight(getGlobalHeight());
        getPlayer().setBrimstoneStage(1);
        //Set Location for Boss Npc
        setCurrentNpc(new NPC(npcId(), new Position(3874, 2783, getMyHeight())));
        ...
        TeleportHandler.teleportPlayer(getPlayer(), new Position(3873, 2777, getMyHeight()), TeleportType.NORMAL);
    }
