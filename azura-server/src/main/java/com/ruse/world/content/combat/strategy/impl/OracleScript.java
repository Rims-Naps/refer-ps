...
public class OracleScript implements CombatStrategy {
    ...
    private void rangeAttack(Player player, NPC boss) {
        ...
        for (Player p : World.getNearbyPlayers(boss.getPosition(), 20)) {
            ...
            if (prayeroff == 1) {
                p.msgRed("Oracle disabled your prayers..");
            ...
            TaskManager.submit(new Task(5, p, false) {
                ...
                public void execute() {
                    if (p.getCurseActive()[CurseHandler.BLOCK_RANGE]) {
                        ...
                        p.sendMessage("<shad=0>@gre@You successfully blocked Oracles's range attack");
                    } else {
                        p.dealDamage(new Hit(120, Hitmask.CRITICAL, CombatIcon.RANGED));
                        p.sendMessage("<shad=0>@red@You failed to block Oracles's range attack");
                    }
                    rangeRunning = false;
                    this.stop();
                }
            ...
            TaskManager.submit(new Task(5, p, false) {
                ...
                public void execute() {
                    if (p.getCurseActive()[CurseHandler.BLOCK_MAGIC]) {
                        p.dealDamage(new Hit(0, Hitmask.NONE, CombatIcon.BLOCK));
                        p.sendMessage("<shad=0>@gre@You successfully blocked Oracle's Magic attack");
                    } else {
                        p.dealDamage(new Hit(120, Hitmask.CRITICAL, CombatIcon.MAGIC));
                        p.msgRed("You failed to block Oracle's Magic attack");
                    }
                    mageRunning = false;
                    this.stop();
                }
    ...
    private void meleeAttack(Player player, NPC boss) {
        ...
        if (prayeroff == 1) {
            ...
            player.msgRed("The Oracle disabled your prayers!");
        }
        ...
        TaskManager.submit(new Task(5, player, false) {
            ...
            public void execute() {
                    if (player.getCurseActive()[CurseHandler.BLOCK_MELEE]) {
                        ...
                        player.sendMessage("<shad=0>@gre@You successfully blocked Oracle's Melee Attack");
                    } else {
                        player.dealDamage(new Hit(120, Hitmask.CRITICAL, CombatIcon.MELEE));
                        player.sendMessage("<shad=0>@red@You failed to block Oracle's Melee Attack");
                }
                meleeRunning = false;
                this.stop();
