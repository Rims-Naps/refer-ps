...
public class Hunter {
	...
	public static boolean goodArea(Player client) {
		if (client.getLocation() == Locations.Location.DUEL_ARENA){
			return true;
		}
		int x = client.getPosition().getX();
		int y = client.getPosition().getY();
	...
	public static int getMaximumTraps(Player client) {
		return client.getSkillManager().getCurrentLevel(Skill.BEAST_HUNTER) / 20 + 1;
	}
	...
	public static void layTrap(Player client, Trap trap) {
		...
		if (trap instanceof BoxTrap) {
			id = 10008;
			if (client.getSkillManager().getCurrentLevel(Skill.BEAST_HUNTER) < 60) {
		...
		if (canLay(client)) {
			...
			if (trap instanceof SnareTrap) {
				...
				if (client.getSkillManager().getCurrentLevel(Skill.BEAST_HUNTER) < 27) {
					...
					if (theTrap.getOwner() == client) {
						if (theTrap instanceof SnareTrap) {
							...
							if (theTrap.getGameObject().getId() == 19180) {
								if (client.getSkillManager().skillCape(Skill.BEAST_HUNTER)) {
									client.getInventory().add(10088, 40 + Misc.getRandom(30) + Misc.getRandom(30));
								} else {
									client.getInventory().add(10088, 20 + Misc.getRandom(30));
								}
								client.getPacketSender().sendMessage("You have succesfully caught a Crimson Swift.");
								client.getSkillManager().addExperience(Skill.BEAST_HUNTER, (int) (exps[0]));
								...
								client.getPacketSender().sendMessage("You have succesfully caught a Golden Warbler.");
								client.getSkillManager().addExperience(Skill.BEAST_HUNTER, (int) (exps[1]));
								...
								client.getPacketSender().sendMessage("You have succesfully caught a Copper Longtail.");
								client.getSkillManager().addExperience(Skill.BEAST_HUNTER, (int) (exps[2]));
								...
								client.getPacketSender().sendMessage("You have succesfully caught a Cerulean Twitch.");
								client.getSkillManager().addExperience(Skill.BEAST_HUNTER, (int) (exps[3]));
								...
								client.getPacketSender().sendMessage("You have succesfully caught a Tropical Wagtail.");
								client.getSkillManager().addExperience(Skill.BEAST_HUNTER, (int) (exps[4]));
							}
							if (client.getSkillManager().skillCape(Skill.BEAST_HUNTER)) {
							...
							if (theTrap.getGameObject().getId() == 19191) {
								...
								client.getSkillManager().addExperience(Skill.BEAST_HUNTER, (int) (exps[6]));
								client.getPacketSender().sendMessage("You have succesfully caught a Chinchompa!");
								...
								client.getSkillManager().addExperience(Skill.BEAST_HUNTER, (int) (exps[7]));
								client.getPacketSender().sendMessage("You have succesfully caught a Red Chinchompa!");
							}
						}
						if (client.getSkillManager().skillCape(Skill.BEAST_HUNTER)) {
							client.getPacketSender().sendMessage("Your cape gives you double the loot!");
	...
	public static void catchNPC(Trap trap, NPC npc) {
		if (trap.getTrapState() == TrapState.CAUGHT)
			return;
		if (trap.getOwner() != null) {
			if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.BEAST_HUNTER) < requiredLevel(npc.getId())) {
