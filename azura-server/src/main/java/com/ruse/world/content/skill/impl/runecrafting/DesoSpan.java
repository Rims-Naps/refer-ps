...
public class DesoSpan {
	...
	enum Energy {
		GREEN_ENERGY(5, 1, 66, 912, 551, 999),
		YELLOW_ENERGY(6, 80, 160, 913, 554, 1006);
	...
	public static void siphon(Player player, NPC n) {
		Energy energyType = Energy.forId(n.getId());
		...
		if (energyType != null) {
			...
			player.setCurrentTask(new Task(cycle, player, false) {
				...
				public void execute() {
					...
					n.dealDamage(new Hit(Misc.getRandom(12), Hitmask.NEUTRAL, CombatIcon.MAGIC));
					if (Misc.getRandom((30 + gearboost(player))) <= 10) {
						player.dealDamage(new Hit(1 + Misc.getRandom(48), Hitmask.NEUTRAL, CombatIcon.DEFLECT));
