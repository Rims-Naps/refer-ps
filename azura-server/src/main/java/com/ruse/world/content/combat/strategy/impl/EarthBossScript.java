...
public class EarthBossScript implements CombatStrategy {
    public int npcId() {
        return 8002;
    }
    public void regularAttack(Player player, NPC boss) {
        ...
        new Projectile(boss, player, 1000, 75, 10, 110, 0, 0).sendProjectile();
    }
    ...
    public boolean customContainerAttack(Character entity, Character victim) {
        ...
        if (attack1) {
            boss.forceChat("Gaia's wrath will be felt!!!");
            sendRandomGraphics(boss, new Graphic(839, 4), 12, 18);
            for (Player p: World.getNearbyPlayers(boss.getPosition(), 20)) {
        ...
        if (attack2) {
            sendRandomGraphics(boss, new Graphic(405  , 4), 12, 18);
            ...
            for (Player p : World.getNearbyPlayers(boss.getPosition(), 20)) {
                ...
                TaskManager.submit(new Task(6, p, false) {
                    @Override
                    public void execute() {
                        sendRandomGraphics(boss, new Graphic(8 ), 12, 18);
                        p.getPacketSender().sendCameraNeutrality();
                        this.stop();
                    }
                });
