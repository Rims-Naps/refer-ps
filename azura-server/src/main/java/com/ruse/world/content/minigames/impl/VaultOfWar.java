...
public class VaultOfWar {
    ...
    public static final int AVATAR_ID = 1208;
    ...
    public static void enterEasternDungeon(Player player) {
        if (player.getInventory().contains(TOKEN_ID) && player.getInventory().getAmount(TOKEN_ID) < 100) {
    ...
    public static void handleTokenRemoval(Player player) {
        ...
        TaskManager.submit(new Task(5, player, false) {
            ...
            public void execute() {
                ...
                if (player.getInventory().contains(TOKEN_ID)) {
                    ...
                    player.moveTo(new Position(1770, 5333, 0));
                    resetTokenRoomVariables(player);
    ...
    private static boolean hasAllCombatRings(Player player) {
        ...
        for (int i = 0; i < COMBAT_GLOVES.length; i++) {
            if (player.getInventory().contains(COMBAT_GLOVES[i])*/
/* || player.getEquipment().contains(COMBAT_GLOVES[i])*//*
) {
                if (i == 0)
                    ring1 = true;
                else if (i == 1)
