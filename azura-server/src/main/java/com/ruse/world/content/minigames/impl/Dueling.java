...
public class Dueling {
	...
	public void stakeItem(int itemId, int amount, int slot) {
		...
		if (playerToDuel.getGameMode() == GameMode.IRONMAN || player.getGameMode() == GameMode.EZMODE || playerToDuel.getGameMode() == GameMode.GROUP_IRON) {
	...
	public static boolean handleDuelingButtons(Player player, int button) {
		if (DuelRule.forButtonId(button) != null) {
			...
			if (player.getDueling().duelingWith < 0)
				return false;
			Player playerToDuel = World.getPlayers().get(player.getDueling().duelingWith);
			switch (button) {
			case 6674:
