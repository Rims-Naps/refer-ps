...
public class Trading {
	...
	public void requestTrade(Player player2) {
		...
		if (player2.getGameMode() == GameMode.IRONMAN) {
            if (!player.getRights().isManagement() && player.getRights() != PlayerRights.YOUTUBER) {
                player.getPacketSender().sendMessage("That player is of the ironman type and cannot trade.");
                return;
            }
		}

        if (player.getGameMode() == GameMode.IRONMAN) {
            if (!player2.getRights().isManagement() && player2.getRights() != PlayerRights.YOUTUBER) {
                player.getPacketSender().sendMessage("That player is of the ironman type and cannot trade.");
                return;
            }
        }
        ...
        if (player.getGameMode() == GameMode.GROUP_IRON) {
            if (player.getIronmanGroup() == null){
                player.sendMessage("You need to be in group to trade with other group ironmen.");
                return;
            }
            if (!player.getIronmanGroup().getMembers().contains(player2.getName()) && !player2.getRights().isManagement() && player2.getRights() != PlayerRights.YOUTUBER) {
                player.getPacketSender().sendMessage("You can only trade your group members");
                return;
            }
        }

        if (player2.getGameMode() == GameMode.GROUP_IRON) {
            if (player2.getIronmanGroup() == null) {
                player2.sendMessage("You need to be in group to trade with other group ironmen.");
                return;
            }
            if (!player2.getIronmanGroup().getMembers().contains(player.getName()) && !player.getRights().isManagement() && player.getRights() != PlayerRights.YOUTUBER) {
                player.getPacketSender().sendMessage("You can only trade your group members");
                return;
            }
        }
	...
	public void tradeItem(int itemId, int amount, int slot) {
		...
		if(player.getRights() != PlayerRights.DEVELOPER && player2.getRights() != PlayerRights.DEVELOPER && !(itemId == 1419 && player.getRights().isStaff())) {
			if (player.getRights() != PlayerRights.OWNER && player2.getRights() != PlayerRights.OWNER)
				if (player.getRights() != PlayerRights.CO_OWNER && player2.getRights() != PlayerRights.CO_OWNER)
	...
	public void acceptTrade(int stage) {
        ...
        if (player.getInventory().getFreeSlots() < player2.getTrading().offeredItems.size()) {
            player.sendMessage("You require more free space to do this trade.");
            player2.sendMessage("The person you are trading with requires more free space to do this trade");
            return;
        }
        if (player2.getInventory().getFreeSlots() < player.getTrading().offeredItems.size()) {
            player2.sendMessage("You require more free space to do this trade.");
            player.sendMessage("The person you are trading with requires more free space to do this trade");
            return;
        }
		...
		if (stage == 2) {
			...
			if (inTrade() && player2.getTrading().tradeConfirmed2) {
				...
				StringBuilder log = new StringBuilder(
						Misc.formatPlayerName(player2.getUsername()) + " trading with " + Misc.formatPlayerName(player.getUsername()) + "\n"
				);
				log.append("Offered by " + Misc.formatPlayerName(player2.getUsername()) + ":\n```\n");
				for (Item item : player2.getTrading().offeredItems) {
					log.append("(" + item.getDefinition().getName() + " x" + item.getAmount() + ", ID " + item.getId() + ")\n");
				}
				log.append("```\nOffered by " + Misc.formatPlayerName(player.getUsername()) + ":\n```\n");
				for (Item item : offeredItems) {
					log.append("(" + item.getDefinition().getName() + " x" + item.getAmount() + ", ID " + item.getId() + ")\n");
				}
				log.append("```");

				String[] tradeMessages = log.toString().split("\n");
				for (String string : tradeMessages) {
					DiscordManager.sendMessage(string, Channels.TRADING);
					if (WatchListManager.beingWatched(player.getUsername()) || WatchListManager.beingWatched(player2.getUsername())) {
						DiscordManager.sendMessage(string, Channels.WATCHLIST_TRADES);
					}
				}
				giveItems();
	...
	public void giveItems() {
		...
		try {
			...
			if (player.getHostAddress().contains(player2.getHostAddress()) || player.getMac().contains(player2.getMac())){
				DiscordManager.sendMessage("Same IP Addresses detected during trade: " + "names: " + player.getName() + " " + player.getHostAddress() + " " + player.getMac() + " and " + player2.getName() + " " + player2.getMac() + " " + player2.getHostAddress(), Channels.PUNISHMENTS);
			}
