...
public class Graveyard {
	...
	private static void spawn(Player player, int wave, int level) {
		if (level == 10) {
			leave(player);
	...
	private static Position getSpawnPos(int z) {
	...
	private static int getSpawn(int level) {
		int random = Misc.getRandom(14);
		switch (level) {
		case 1:
			if (random <= 2)
				return 76;
