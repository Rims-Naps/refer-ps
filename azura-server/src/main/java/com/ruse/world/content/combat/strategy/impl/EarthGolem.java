...
public class EarthGolem implements CombatStrategy {
                ...
                public void execute() {
                    if (!otherVictims.is_dead_in_tower()) {
                        ...
                        if (!otherVictims.getCurseActive()[CurseHandler.BLOCK_RANGE]) {
                            otherVictims.dealDamage(new Hit(Misc.random(5 * modifier, 25* modifier), Hitmask.EARTH, CombatIcon.RANGED));
                        }
                        if (otherVictims.getCurseActive()[CurseHandler.BLOCK_RANGE]) {
                            otherVictims.dealDamage(new Hit(Misc.random(0, 5 * modifier), Hitmask.EARTH, CombatIcon.BLOCK));
                        }
                    }
                    this.stop();
                }
