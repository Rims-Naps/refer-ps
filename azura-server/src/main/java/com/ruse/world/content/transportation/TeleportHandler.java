...
import static com.ruse.world.content.SlayerDungeonInstance.destroySlayerDungeonInstance;


public class TeleportHandler {

	public static void teleportPlayer(Player player, Position targetLocation,
		...
		/*if (player.isInRaid()) {
			player.sendMessage("You can not teleport out of raids");
			return;
		}*/
		if (player.getPosition().getRegionId() == 13365 || player.getPosition().getRegionId() == 13364|| player.getPosition().getRegionId() == 13363){
			...
			player.getPacketSender().sendTab(15);
		}

		if (player.getPosition().getRegionId() == 9279 || player.getPosition().getRegionId() == 9535|| player.getPosition().getRegionId() == 9277){
	...
	/*	final RaidsParty party = player.getMinigameAttributes().getRaidsAttributes().getParty();
		...
		/*if (player.getLocation() == Locations.Location.VENGEANCE_ENTRANCE) {
			player.sendMessage("You can not teleport out of raids");
			return;
		}*/
		if (player.getForgottenRaidParty() != null) {
			player.sendMessage("Please leave your party by doing ::leave to exit the raid.");
			return;
		}
			...
			public void execute() {
				...
				default:
					if (tick == teleportType.getStartTick()) {
						...
						player.getPacketSender().sendFadeTransition(250, 100, 150);
						if (Dungeoneering.doingOldDungeoneering(player)) {
							Position dungEntrance = player.getMinigameAttributes().getDungeoneeringAttributes()
	...
	public static void onArrival(Player player, Position targetLocation) {
		if (targetLocation.getX() == 3683 && targetLocation.getY() == 9888) {
		}
	}
