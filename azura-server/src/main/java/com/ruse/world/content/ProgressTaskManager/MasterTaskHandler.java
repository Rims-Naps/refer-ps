...
public class MasterTaskHandler {
    ...
    public MasterTaskHandler(Player player) {
        this.player = player;
    }

    public void openInterface() {
        int index = 148052;
        for (MasterTasks.MasterTaskData data : MasterTasks.MasterTaskData.values()) {
            ...
            player.getPacketSender().sendString(148003, "Master Tasks");
        ...
        player.getPacketSender().setScrollBar(148050, 2 + (MasterTasks.MasterTaskData.values().length * 60));
    ...
    public boolean hasCompletedAll() {
        int amount = 0;
        for (MasterTasks.MasterTaskData masterTask : MasterTasks.MasterTaskData.values()) {
            if (isComplete(masterTask.ordinal()))
                amount++;
        }
        return amount == MasterTasks.getMaxTasks();
    }
    ...
    private void complete(int masterTask) {
        for (MasterTasks.MasterTaskData ach : MasterTasks.MasterTaskData.values()) {
            if (masterTask == ach.ordinal()) {
                MasterTasks.claimReward(player, ach);
            }
        }
