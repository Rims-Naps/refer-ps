...
public class VotingStreak {
    ...
    public void openInterface() {
        ...
        player.getPacketSender().sendString(149006, "@gre@" + Misc.getTimeTillReset());

        int inter = 149008;

        for (int i = 0; i < 30; i++) {
            player.getPacketSender().sendSpriteChange(inter++, daysVoted[i] ? 3362 : currentDay == i ? 3361 : 3541);
            inter++;
        ...
        player.sendMessage("Voting streak status: "
                + (daysVoted[currentDay] ? "<col=02940e><shad=0>You already claimed the daily reward today" : "@red@You must Vote to claim the reward!"));
    }
