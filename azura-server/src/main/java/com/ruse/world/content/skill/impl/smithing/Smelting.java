...
public class Smelting {
	...
	public static void smeltBar(Player player, int barId, int amount) {
	...
	public static void handleBarCreation(int barId, Player player) {
		if (player.getOres()[0] > 0) {
			...
			if (player.getOres()[1] > 0 && player.getOres()[1] != 453) {
				...
				if (player.getSkillManager().skillCape(Skill.JOURNEYMAN) && Misc.getRandom(3) == 1) {
			...
			if (barId != 2351) { // Iron bar - 50% successrate
				...
				player.getSkillManager().addExperience(Skill.JOURNEYMAN, getExperience(barId));
