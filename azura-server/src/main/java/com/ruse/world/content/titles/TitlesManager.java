    ...
    private void loadInfo() {
        ...
        if (previewingTitle != null) {
            ...
            if (previewingTitle.getType() == Titles.TitleType.BOSS) {
                player.getPacketSender().sendString(152019, ("@red@") + previewingTitle.getRequirements()[0]);
...
/*
    ...
    public void checkBossTitles() {
        for (Titles title : Titles.values()) {
            if (title.getType() == Titles.TitleType.BOSS) {
                if (KillsTracker.getTotalKillsForNpc(title.getNpcId(), player) >= title.getAmount()) {
                    unlock(title);
                }
            }
        }
    ...
    public void checkMiscTitles() {
        ...
        if (getObtainedTitles().contains(Titles.GRINDER) && player.getTotalPlayTime() < 2000){
            getObtainedTitles().remove(Titles.GRINDER);
        }
        if (getObtainedTitles().contains(Titles.NOLIFE) && player.getTotalPlayTime() < 5000){
    ...
    private boolean checkMiscTitle(Titles title) {
        ...
        if (title == Titles.GRINDER) {
            if (player.getTotalPlayTime() >= 2000)
                return true;
        }
        if (title == Titles.NOLIFE) {
            if (player.getTotalPlayTime() >= 5000)
                return true;
        }
