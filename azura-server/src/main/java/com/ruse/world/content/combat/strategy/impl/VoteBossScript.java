...
public class VoteBossScript implements CombatStrategy {
    ...
    private void sendStage(NPC boss, int execStage) {
        ...
        switch (execStage) {
            case 0:
                GameSettings.votebossStage = 3;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(190));
                    boss.forceChat("I'll defeat you all!");
                }
                break;
            case 1:
                GameSettings.votebossStage = 1;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(190));
                }
                break;
            case 2:
                GameSettings.votebossStage = 2;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(190));
                }
                break;
