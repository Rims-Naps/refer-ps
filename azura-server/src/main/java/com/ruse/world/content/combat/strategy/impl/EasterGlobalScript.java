...
public class EasterGlobalScript implements CombatStrategy {
    ...
    public boolean customContainerAttack(Character entity, Character victim) {
        ...
        boss.performAnimation(new Animation(4230));
        ...
        for(Player otherVictims : World.getNearbyPlayers(boss.getPosition(), 25)) {
            new Projectile(boss, otherVictims, 711, 75, 10, 110, 0, 0).sendProjectile();
            ...
            TaskManager.submit(new Task(3, otherVictims, false) {
                ...
                public void execute() {
                    if (!otherVictims.getCurseActive()[CurseHandler.BLOCK_RANGE]) {
                        otherVictims.dealDamage(new Hit(Misc.random(5, 15), Hitmask.VOID, CombatIcon.RANGED));
                    }
                    if (otherVictims.getCurseActive()[CurseHandler.BLOCK_RANGE]) {
                        otherVictims.dealDamage(new Hit(Misc.random(0), Hitmask.VOID, CombatIcon.BLOCK));
                    }
        ...
        switch (execStage) {
            case 0:
                ...
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(1044));
                }
                break;
            case 1:
                GameSettings.donobossStage = 1;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(1045));
                }
                break;
            case 2:
                GameSettings.donobossStage = 2;
                for (Player victims : World.getNearbyPlayers(boss.getPosition(), 20)) {
                    victims.performGraphic(new Graphic(2036));
                }
                break;
        }
    }
