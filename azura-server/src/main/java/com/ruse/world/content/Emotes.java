...
public class Emotes {
	...
	public enum EmoteData {
		...
		SCARED(15166, new Animation(11044), new Graphic(1973) ),
	...
	public static boolean doEmote(Player player, int buttonId) {
		...
		if (emoteData != null) {
			...
			if (data != null) {
				...
				if (!player.getEmoteDelay().elapsed(data.delay * 1000L)) {
					player.getPacketSender()
							.sendMessage("You must wait a bit before performing another skillcape emote.");
					return true;
				}
				boolean lock = player.getMovementQueue().isLockMovement();
				...
				player.getPacketSender().sendMessage("You must be wearing a Vanguard Cape in order to use this emote.");
			return true;
		}
		return false;
	}
	...
	private enum Skillcape_Data {
		...
		SLAYER(new int[] { 9786, 9787, 21613, 10653 }, 4967, 1656, 8), FARMING(new int[] { 9810, 9811, 10661 }, 4963, -1, 16),
		RUNECRAFTING(new int[] { 9765, 9766, 10645 }, 4947, 817, 10),
		CONSTRUCTION(new int[] { 9789, 9790, 10654 }, 4953, 820, 16),
		BEAST_HUNTER(new int[] { 9948, 9949, 10646 }, 5158, 907, 14),
