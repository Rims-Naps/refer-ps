...
public class CorruptHardRaidChest {
    private static final int KEY_ID = 792;

    private static final  int[] TRINKET_MATERIAL = new int[] {431};
    private static final  int[] UPGRADE_MATERIALS = new int[] {454,455,3007,2619};
    private static final  int[] RING_RARE = new int[] {23057,15670};
    private static final  int[] SUPER_RARE = new int[] {418, 417, 10256, 10260, 10262, 15668, 16420,17049, 17050, 17051, 17052, 17053};
    private static final int[] RARE = new int[] {10945,15668,17130};
    private static final int[] UNCOMMON = new int[] {416, 417, 730, 406, 407, 408, 409, 410};
    private static final int[] COMMON = new int[] { 15667, 1446, 20108, 20109,20111, 17582,17586,17584};
    ...
    public static void generateReward(Player player){
        ...
        if (Misc.getRandom(700) == 1) {
            int trinketrare = getRandomItem(TRINKET_MATERIAL);

            player.getInventory().addDropIfFull(trinketrare, 1);
            World.sendMessage("<col=AF70C3><shad=0>[Raids]<shad=0>@red@ " + player.getUsername() + " <col=AF70C3><shad=0>pulled a <shad=0>@red@" +  ItemDefinition.forId(trinketrare).getName() + " <col=AF70C3><shad=0>from the Raid Chest!");
            player.sendMessage("@bla@<shad=0>You received a @red@<shad=0>"  + ItemDefinition.forId(trinketrare).getName() + " @bla@<shad=0>from the chest!");
            player.getCollectionLogManager().addItem(CollectionLogs.CORRUPT_RAID_HARD, new Item(trinketrare));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }

        else if (Misc.getRandom(400) == 1) {
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.CORRUPT_RAID_HARD, new Item(axerares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.CORRUPT_RAID_HARD, new Item(ringRares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.CORRUPT_RAID_HARD, new Item(superrares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.CORRUPT_RAID_HARD, new Item(rares));

        }
        else if (Misc.inclusiveRandom(12) == 1) {
            int uncommons = getRandomItem(UNCOMMON);
            player.getInventory().addDropIfFull(uncommons, Misc.random(1,2));
            player.getCollectionLogManager().addItem(CollectionLogs.CORRUPT_RAID_HARD, new Item(uncommons));


        } else {
            int commons = getRandomItem(COMMON);
            player.getInventory().addDropIfFull(commons, 1);
            player.getInventory().addDropIfFull(995, Misc.random(40,100));
            player.getCollectionLogManager().addItem(CollectionLogs.CORRUPT_RAID_HARD, new Item(commons));
