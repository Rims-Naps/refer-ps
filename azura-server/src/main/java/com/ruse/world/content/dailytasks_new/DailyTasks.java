...
public class DailyTasks {
    ...
    public static void checkTask(Player player) {
        ...
        } else if (!player.dailies.isEmpty()) {
    ...
    public static void assign(Player player, TaskChallengeTimespan timespan) {
        int freeIndex = findFreeIndexDaily(player.dailies);
        if (freeIndex == -1) {
        ...
        DailyTask[] tasks = player.skillingTask ? DailyTask.SKILLING_TASKS : DailyTask.PVM_TASKS;
        DailyTask selected = Misc.random(tasks);

        while(player.dailies.containsKey(selected)) {
            selected = Misc.random(tasks);
        }

        TaskChallenge task = createTask(freeIndex, selected, timespan);
        player.dailies.put(selected, task);
        task.send(player);
    ...
    public static void sendProgress(Player player) {
        ...
        player.dailies.forEach((task, challenge) -> {
            challenge.sendInfo(player);
        });
    }

    private static TaskChallenge createTask(int index, DailyTask task, TaskChallengeTimespan time) {
        int assignedAmount = Misc.random(task.assignmentMin, task.assignmentMax);
        TaskChallenge session = new TaskChallenge();
        session.key = task;
    ...
    public static void progress(Player player, DailyTask task) {
        TaskChallenge dts = player.dailies.get(task);
        if (dts == null || dts.completed())
            return;

        int oldVal = dts.completedAmount;
        dts.increment();
    ...
    public static void scanForExpired(Player player) {
        ...
        if (!removed.isEmpty()) {
            removed.forEach(index -> player.dailies.remove(index.key));
        } else {
    ...
    public static void claimReward(Player player) {
        TaskChallenge task = player.dailies.values().stream()
                .filter(t -> t.index == 0)
                .findAny().orElse(null);
