...
public class PuroPuro {
	 ...
	 */
	public static void catchImpling(Player player, NPC imp) {
		...
		if (player.getInterfaceId() > 0 || player == null || imp == null || implingData == null || !imp.isRegistered()
			...
			return;
		if (player.getSkillManager().getCurrentLevel(Skill.BEAST_HUNTER) < implingData.levelReq) {
		...
		boolean sucess = player.getSkillManager().getCurrentLevel(Skill.BEAST_HUNTER) > 8
				? Misc.getRandom(player.getSkillManager().getCurrentLevel(Skill.BEAST_HUNTER) / 2) > 1
				: true;
		if (sucess) {
			if (imp.isRegistered()) {
				...
				if (player.getSkillManager().skillCape(Skill.BEAST_HUNTER)) {
				...
				player.getSkillManager().addExperience(Skill.BEAST_HUNTER,
	 ...
	 */
	public static void goThroughWheat(Player player, GameObject object) {
	 ...
	 */
	public static void lootJar(Player player, Item jar, JarData jarData) {
