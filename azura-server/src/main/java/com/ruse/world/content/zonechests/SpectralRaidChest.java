...
public class SpectralRaidChest {
    private static final int KEY_ID = 2053;

    private static final  int[] BOW_RARE = new int[] {2079};
    ...
    private static final int[] UNCOMMON = new int[] {417,416,};
    ...
    public static void generateReward(Player player){
        ...
        if (Misc.getRandom(400) == 1) {
            int axerares = getRandomItem(BOW_RARE);
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID, new Item(axerares));
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID, new Item(ringRares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID, new Item(superrares));
            player.getInventory().addDropIfFull(2617, 1);
            player.msgFancyPurp("You have received a Skill point while on your Journey.");
        }
            ...
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID, new Item(rares));

        }
        else if (Misc.inclusiveRandom(12) == 1) {
            int uncommons = getRandomItem(UNCOMMON);
            player.getInventory().addDropIfFull(uncommons, 1);
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID, new Item(uncommons));

        } else {
            int commons = getRandomItem(COMMON);
            player.getInventory().addDropIfFull(commons, 1);
            player.getInventory().addDropIfFull(995, Misc.random(50,400));
            player.getCollectionLogManager().addItem(CollectionLogs.SPECTRAL_RAID, new Item(commons));
