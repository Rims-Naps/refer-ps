...
public class ClanChatManager {
	...
	public static void sendMessage(Player player, String message) {
		...
		DiscordManager.sendMessage("[Clanchat] " + player.getName() + ": " + NameUtils.capitalize(message), Channels.CLANCHAT);
		...
		for (Player memberPlayer : clan.getMembers()) {
			if (memberPlayer != null) {
				if (memberPlayer.getRelations().getIgnoreList().contains(player.getLongUsername()))
					continue;
				int img = Misc.spriteForRights(player.getRights().ordinal());
				int ironimg = Misc.spriteForIronman(player.getGameMode().ordinal());
                int diffImg = Misc.spriteForDiff(player.getXpmode().ordinal());

				String formatted = String.format("%02d", clan.getName().length() + 1);

				String rankImg = img > 0 ? " <img=" + img + ">" : "";
                String rankiron = ironimg > 0 ? " <img=" + ironimg + ">" : "";
                String rankDiff = diffImg > 0 ? " <img=" + diffImg + ">" : "";
				memberPlayer.getPacketSender().sendMessage(":clan:" + formatted +    "[" +  clan.getName() +  "]" + rankImg + rankDiff + rankiron + " " + NameUtils.capitalizeWords(player.getUsername()) + ": "
						+ /*
				 * chatColor +
		...
		if (discordListening) {
			String formattedMessage = String.format("[Clan Chat] %s: %s", player.getUsername(), message);
			DiscordManager.sendMessage(formattedMessage, Channels.CLANCHAT ); // Replace with your actual Discord channel ID
		}
	...
	private static boolean discordListening = false;
	public static void setDiscordListening(boolean listening) {
		discordListening = listening;
	}

	public static boolean isDiscordListening() {
		return discordListening;
	}
	...
	private static void join(Player player, ClanChat clan) {
		...
		player.getPacketSender().sendMessage(" Entered " + clan.getOwnerName() + "'s clan chat.");
