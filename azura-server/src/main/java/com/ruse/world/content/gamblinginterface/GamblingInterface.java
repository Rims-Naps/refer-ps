...
public class GamblingInterface {
    ...
    public void requestGamble(Player player2) {
        ...
        if (player.getHostAddress().equals(player2.getHostAddress())) {
            ...
            World.sendStaffMessage("@bla@<shad=0> " + player.getUsername() + " @bla@<shad=0> TRIED TO GAMBLE THEMSELVES@bla@<shad=0> " + player2.getUsername());
            return;
        }
        ...
        if (inGamble()) {
            System.out.println("p is in gamble");
            declineGamble(true);
            return;
        }
        ...
        //System.out.println("player2 index   " + player2.getIndex());
        ...
        //System.out.println("inGamble(): " + inGamble());
        //System.out.println("P2 Gamble requested: " + player2.getGambling().gambleRequested());
        //System.out.println("P2 getGambleWith: " + player2.getGambling().getGambleWith());
        //System.out.println("P1.getIndex(): " + player.getIndex());

        System.out.println("Player: " + player.getUsername() + " | Index: " + player.getIndex() + " | inGamble: " + inGamble());
        System.out.println("Target: " + player2.getUsername() + " | Index: " + player2.getIndex() + " | gambleRequested: " + player2.getGambling().gambleRequested());
        System.out.println("target gambling with: " + player2.getGambling().getGambleWith());
    ...
    public void openGamble() {
        ...
        setCanOffer(true);
        setGambleStatus(1);
        setGamblingMode(GamblingMode.FLOWER_POKER); // this
    ...
    public void gambleItem(int itemId, int amount, int slot) {
        ...
        if (player2 == null || player == null)
            return;
        if (itemId != 19048) {
            player.sendMessage("You can only gamble Athens shards!");
            return;
        }
    ...
    public void acceptGamble(int stage) {
        ...
        if (getGambleWith() < 0) {
            System.out.println(getGambleWith() + " = getGambleWith()");
            declineGamble(false);
            return;
        }
        ...
        if (player == null || player2 == null) {
            System.out.println("player or player2 == null");
            declineGamble(false);
            return;
        }
    ...
    private void sendDescriptionForMode(GamblingMode mode) {
        ...
        if (mode == GamblingMode.DICE_DUEL) {
        ...
        } else if (mode == GamblingMode.FLOWER_POKER) {
        ...
        } else if (mode == GamblingMode.BLACKJACK) {
        ...
        } else if (mode == GamblingMode.FIFTY_FIVE_X2) {
            player.getPacketSender().sendString(57157, "@whi@Ye this one is simple")
    ...
    private void startGame(GamblingMode mode) {
        ...
        if (mode == GamblingMode.FLOWER_POKER) {
            startFlowerPoker(false);
        }
    }

    private List<Integer> flowerList = new ArrayList<>();

    private void startFlowerPoker(boolean tie) {
        ...
        if (tie) {
            Position target = player.getPosition().copy();
            target.setY(2676);
            player.moveTo(target);
            Position target2 = player2.getPosition().copy();
            target2.setY(2676);
            player2.moveTo(target2);
        } else {
            if (!playersGamblingInArea(2966, 2672, 2967, 2676)) {
                player.moveTo(new Position(2966, 2676));
                player2.moveTo(new Position(2967, 2676));
            } else if (!playersGamblingInArea(2971, 2672, 2972, 2676)) {
                player.moveTo(new Position(2971, 2676));
                player2.moveTo(new Position(2972, 2676));
            } else if (!playersGamblingInArea(2976, 2672, 2977, 2676)) {
                player.moveTo(new Position(2976, 2676));
                player2.moveTo(new Position(2977, 2676));
            } else if (!playersGamblingInArea(2981, 2672, 2982, 2676)) {
                player.moveTo(new Position(2981, 2676));
                player2.moveTo(new Position(2982, 2676));
            } else {
                declineGamble(false);
                player2.getGambling().declineGamble(false);
                player.getPacketSender().sendMessage("All fp slots are busy right now. Please try again in a minute.");
                player2.getPacketSender().sendMessage("All fp slots are busy right now. Please try again in a minute.");
                return;
            }
        }
        ...
        player2.setPositionToFace(new Position(player2.getPosition().getX(), player2.getPosition().getY() - 1));
    ...
    private static boolean playersGamblingInArea(int x, int y, int x1, int y1) {
        for (Player player : World.getPlayers()) {
            if (player == null) {
                continue;
            }

            //Is the player gambling
            if (player.getGambling().getGamblingMode() == null) {
                continue;
            }

//			//Is the player doing some flower poker
//			if (player.getGambling().getGamblingMode().equals(GamblingMode.FLOWER_POKER)) {
//				continue;
//			}

            //Check that player is in the area of a lane we are checking, if so make that lane invalid for usage.
            Position position = player.getPosition();
            if (position.getX() >= x && position.getX() <= x1 && position.getY() >= y && position.getY() <= y1) {
                return true;
            }
        }
        return false;
    ...
    private void plantFlower() {
        ...
        player.getMovementQueue().reset();
        player.performAnimation(new Animation(827)); // 827
        ...
        GameObject flower1 = new GameObject(flowers1.objectId, player2.getPosition().copy());
    ...
    private void getFPWinner() {
        ...
        if (result1 == result2) {
            ...
            TaskManager.submit(new Task(2) {
                @Override
                protected void execute() {
                    player.forceChat("TIE - Replant starting starting soon...!");
                    player2.forceChat("TIE - Replant starting starting soon...!");
                    stop();
                    return;
                }
            });
            flowerCount = 0;
            TaskManager.submit(new Task(10) { //17
                @Override
                protected void execute() {
                    ...
                    for (GameObject obj : player2.getGambling().plants) {
                        CustomObjects.deleteGlobalObject(obj);
                    }
                    player2.getGambling().plants.clear();
                    startFlowerPoker(true);
                    stop();
                    return;
                }
            });
            ...
            TaskManager.submit(new Task(4) {
                @Override
                protected void execute() {
                    player.forceChat("I win with " + getResult(flowerList) + "!");
                    giveItems();
                    flowerCount = 0;
                    for (GameObject object : plants) {
                        // System.out.println("-executed-");
            ...
            TaskManager.submit(new Task(4) {
                @Override
                protected void execute() {
                    player2.forceChat("I win with " + player2.getGambling().getResult(player2.getGambling().flowerList) + "!");
                    for (GameObject object : plants) {
                        // System.out.println("-executed-");
                        CustomObjects.deleteGlobalObject(object);
                    }
    ...
    public boolean handleChoice(int id) {
        ...
        switch (id) {
            ...
            case -8342:
            case -8340:
                player.getPacketSender().sendConfig(1703, 0).sendConfig(1702, 0).sendConfig(1701, 0).sendConfig(1700, 1);
                player2.getPacketSender().sendConfig(1703, 0).sendConfig(1702, 0).sendConfig(1701, 0).sendConfig(1700, 1);
                setGamblingMode(GamblingMode.FLOWER_POKER);
                player2.getGambling().setGamblingMode(GamblingMode.FLOWER_POKER);
                sendDescriptionForMode(getGamblingMode());
                player2.getGambling().sendDescriptionForMode(player2.getGambling().getGamblingMode());
                player.getPacketSender().sendMessage("This game mode is temporarily disabled.");
                player2.getPacketSender().sendMessage("This game mode is temporarily disabled.");
                break;
