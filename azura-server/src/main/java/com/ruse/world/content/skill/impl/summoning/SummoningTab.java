...
public class SummoningTab {
	...
	public static void handleDismiss(Player plr, boolean dismiss) {
        handleDismiss(plr, dismiss, false);
	}

    public static void handleDismiss(Player plr, boolean dismiss, boolean necro) {
        if(!dismiss && plr.busy()) {
            plr.getPacketSender().sendMessage("Please finish what you're doing first.");
            return;
        }
        if(dismiss) {
            if(plr.getSummoning().getFamiliar() != null) {
                plr.getSummoning().unsummon(true, true);
                plr.setNecrotimeleft(0);
                if (!necro)
                    plr.getPacketSender().sendMessage("You've dismissed your familiar.");
                plr.getPacketSender().sendNpcOnInterface(54021, 60, 1000 ); // 60 = invisable head to remove it
                plr.getPacketSender().sendString(54019, "Boosts:\\n\\n");
                plr.getPacketSender().sendString(54028, "");
                plr.getPacketSender().sendString(54045, "");
            } else {
                plr.getPacketSender().sendMessage("You don't have a familiar to dismiss.");
            }
        } else {
            DialogueManager.start(plr, 4);
            plr.setDialogueActionId(4);
            //plr.getPacketSender().sendInterfaceReset();
        }
    }
	...
	public static void handleSpecialAttack(Player c) {
		...
		if(fam == Summoning.FamiliarData.SPIRIT_WOLF) {
			...
			enemy.setDamage(new Damage(new Hit(Misc.getRandom(40), CombatIcon.MAGIC, Hitmask.NEUTRAL)));
		...
		if(fam == Summoning.FamiliarData.DREADFOWL) {
			...
			enemy.setDamage(new Damage(new Hit(Misc.getRandom(30), CombatIcon.MAGIC, Hitmask.NEUTRAL)));
