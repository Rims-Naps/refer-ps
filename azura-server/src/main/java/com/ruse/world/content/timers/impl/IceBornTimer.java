...
public class IceBornTimer extends EffectTimer {
    Player player;
    public IceBornTimer(Player player) {
        super(player,1465, "Iceborn");
        this.player = player;
    }
    ...
    public void append() {
        ...
        if (isActive() && getTimeLeft() + timeToAdd > 1500) {
            player.getPacketSender().sendMessage("You can only have up to 25 minutes of Iceborn " + getName() + " active");
            return;
        }
