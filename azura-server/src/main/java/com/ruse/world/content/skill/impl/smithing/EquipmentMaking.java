...
public class EquipmentMaking {
	...
	public static void smithItem(Player player, Item bar, Item itemToSmith, int x) {
		...
		if (SmithingData.getData(itemToSmith, "reqLvl") > player.getSkillManager().getCurrentLevel(Skill.JOURNEYMAN)) {
		...
		if (!good2go || !canMakeItem) {
			...
			World.sendStaffMessage("<col=6C1894>[BUG ABUSE] " + player.getUsername() + " just tried to smith: "
		...
		player.setCurrentTask(new Task(3, player, true) {
			...
			public void execute() {
				...
				if (ItemDefinition.forId(itemToSmith.getId()).getName().contains("Tritanium")) {
					player.getSkillManager().addExperience(Skill.JOURNEYMAN,
							(int) (Bars.Tritanium.getExp() * bar.getAmount()));
				...
				// player.getSkillManager().addExperience(Skill.JOURNEYMAN, (int)
