...
public class DailyRewards {
	...
	final int[] day1 = new int[] { 10946, 21204 }; //
	final int[] day2 = new int[] { 6833, 15359 }; // ?
	final int[] day3 = new int[] { 995, 8018 }; //
	final int[] day4 = new int[] { 10946, 23172 }; //
	final int[] day5 = new int[] { 995, 28}; //
	final int[] day6 = new int[] { 15359, 19864 }; // might look confusing but just look at amounts, and ull understand
	final int[] day7 = new int[] { 10946, 19115, 21204, 995, 3687 }; // and to change amounts, here.
	...
	public void claimDay7() {
		...
		if (player.day1Claimed && player.day2Claimed && player.day3Claimed && player.day4Claimed && player.day5Claimed
			...
			player.getInventory().add(21204, 1);
			player.getInventory().add(15359, 1);
			player.getInventory().add(3687, 1);
		...
		switch (buttonId) {
			case -8312:
				...
				if (time < 1 && !player.day1Claimed && player.hasVotedToday) { // true
					...
					player.getInventory().add(10946, 1).add(21204, 2);
			...
			case -8311:
				if (player.day1Claimed && !player.day2Claimed && player.hasVotedToday) { // can claim if day 1 true, day 2
					...
					if (time < 1) {
						...
						player.lastDailyClaim = System.currentTimeMillis() + 43200000;
						player.getInventory().add(6833, 1).add(15359, 1);
						player.hasVotedToday = false;
						return true;
					} else {
			...
			case -8310:
				if (player.day1Claimed && player.day2Claimed && !player.day3Claimed && player.hasVotedToday) {
					if (time < 1) {
						...
						player.lastDailyClaim = System.currentTimeMillis() + 43200000;
						player.getInventory().add(995, 2000).add(8018, 1);
						player.hasVotedToday = false;
						return true;
					} else {
						// System.out.println("Not yet");
			...
			case -8309:
				if (player.day1Claimed && player.day2Claimed && player.day3Claimed && !player.day4Claimed
					...
					if (time < 1) {
						...
						player.lastDailyClaim = System.currentTimeMillis() + 43200000;
						player.getInventory().add(10946, 1).add(23172, 1);
						player.hasVotedToday = false;
						return true;
					} else {
						// System.out.println("Not yet");
			...
			case -8308:
				if (player.day1Claimed && player.day2Claimed && player.day3Claimed && player.day4Claimed
					...
					if (time < 1) {
						...
						player.lastDailyClaim = System.currentTimeMillis() + 43200000;
						player.getInventory().add(995, 2000).add(28, 5);
						player.hasVotedToday = false;
						return true;
					} else {
						// System.out.println("Not yet");
			...
			case -8307:
				if (player.day1Claimed && player.day2Claimed && player.day3Claimed && player.day4Claimed
					...
					if (time < 1) {
						...
						player.lastDailyClaim = System.currentTimeMillis() + 43200000;
						player.getInventory().add(15359, 1).add(19864, 20);
						player.hasVotedToday = false;
						return true;
					} else {
						// System.out.println("Not yet");
	...
	public void displayRewards() {
		...
		for (int t = 0; t < day1.length; t++) {
			player.getPacketSender().sendItemOnInterface(57232, day1[t], t, t == 0 ? 1 : 2);
		}

		for (int y = 0; y < day2.length; y++) {
			player.getPacketSender().sendItemOnInterface(57233, day2[y], y, y == 0 ? 1 : 1);
		}

		for (int u = 0; u < day3.length; u++) {
			player.getPacketSender().sendItemOnInterface(57234, day3[u], u, u == 0 ? 2000 : 1);
		}

		for (int o = 0; o < day4.length; o++) {
			player.getPacketSender().sendItemOnInterface(57235, day4[o], o, o == 0 ? 1 : 1);
		}

		for (int p = 0; p < day5.length; p++) {
			player.getPacketSender().sendItemOnInterface(57236, day5[p], p, p == 0 ? 2000 : 5);
		}

		for (int a = 0; a < day6.length; a++) {
			player.getPacketSender().sendItemOnInterface(57237, day6[a], a, a == 0 ? 1 : 20);
		}
	}
}
