...
public class CombatBuilder {
    ...
    private Map<String, CombatDamageCache> damageMap = new HashMap<>();
    ...
    public Map<String, CombatDamageCache> getDamageMap() {
    ...
    public void attack(Character target) {
        if (character.equals(target)) {
            World.sendStaffMessage("[BUG TRACKER] Error 959.7 has occured. PLEASE REPORT to Hades!");
        ...
        if (character.getInteractingEntity() != target) character.setEntityInteraction(target);
    ...
    public List<DamageDealer> getTopKillers(NPC npc, int numKillers) {
        List<DamageDealer> list = new ArrayList<>();
    ...
    public void addDamage(Character entity, int amount) {
        ...
        if (entity instanceof Player) {
            name = "Player " +((Player) entity).getUsername();
        }
        if (amount < 1 || entity.isNpc()) {
            return;
        }

        //System.out.println("Adding damage for entity: "+name);
        ...
        if (damageMap.containsKey(player.getUsername())) {
            damageMap.get(player.getUsername()).incrementDamage(amount);
            return;
        }

        damageMap.put(player.getUsername(), new CombatDamageCache(amount));
    }
    ...
    public void setContainer(CombatContainer customContainer) {
        ...
        } else this.container = customContainer;
    }
