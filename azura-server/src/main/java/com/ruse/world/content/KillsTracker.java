...
public class KillsTracker {
	...
	public static void open(Player player) {
		try {
			...
			player.getPacketSender().sendString(8147, "<col=AF70C3>-@whi@ Boss kills");
			int index = 1;
			for (KillsEntry entry : player.getKillsTracker()) {
				if (entry.boss) {
					int toSend = 8147 + index > 8196 ? 12174 + index : 8147 + index;
					player.getPacketSender().sendString(toSend, "<col=AF70C3> " + entry.npcName + ": " + entry.amount + "");
					index++;
				}
			}
			index += 1;
	...
	public static KillsEntry entryForID(Player player, int npcId, boolean boss) {
		...
		KillsEntry entry = new KillsEntry(npcId, 0, boss);
