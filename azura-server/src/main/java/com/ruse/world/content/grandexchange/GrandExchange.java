...
public class GrandExchange {
		...
		if (player.getInterfaceId() == MAIN_INTERFACE) {
			...
			case -11736:
				...
				if (player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
					...
					GrandExchangeOffer offer = player.getGrandExchangeSlots()[player.getSelectedGeSlot()]
							.getOffer();
					if (offer.getAmountFinished() < offer.getAmount()) {
						if (offer.getType() == OfferType.BUYING) {
							offer.setCoinsCollect(
	...
	public static ArrayList<GrandExchangeOffer> getOffers(Player player) {
		ArrayList<GrandExchangeOffer> list = new ArrayList<>();
		for (int i = 0; i < player.getGrandExchangeSlots().length; i++) {
			GrandExchangeSlot slot = player.getGrandExchangeSlots()[i];
			if (slot == null)
				continue;
