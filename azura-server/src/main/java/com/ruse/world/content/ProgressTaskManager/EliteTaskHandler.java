...
public class EliteTaskHandler {
    ...
    public EliteTaskHandler(Player player) {
        this.player = player;
    }

    public void openInterface() {
        int index = 148052;
        for (EliteTasks.EliteTaskData data : EliteTasks.EliteTaskData.values()) {
            ...
            player.getPacketSender().sendString(148003, "Elite Tasks");
        ...
        player.getPacketSender().setScrollBar(148050, 2 + (EliteTasks.EliteTaskData.values().length * 60));
    ...
    public boolean hasCompletedAll() {
        int amount = 0;
        for (EliteTasks.EliteTaskData eliteTask : EliteTasks.EliteTaskData.values()) {
            if (isComplete(eliteTask.ordinal()))
                amount++;
        }
        return amount == EliteTasks.getMaxTasks();
    }
    ...
    private void complete(int eliteTask) {
        for (EliteTasks.EliteTaskData ach : EliteTasks.EliteTaskData.values()) {
            if (eliteTask == ach.ordinal()) {
                EliteTasks.claimReward(player, ach);
            }
        }
