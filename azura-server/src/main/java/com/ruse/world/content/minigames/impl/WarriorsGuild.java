...
/**
	...
	public static boolean itemOnAnimator(Player player, Item item, GameObject object) {
				...
				for (int f = 0; f < ARMOR_DATA[0].length; f++) {
					if (item.getId() == ARMOR_DATA[i][f]) {
						if (player.getInventory().contains(ARMOR_DATA[i][0])
							...
							int[] items = new int[] { ARMOR_DATA[i][0], ARMOR_DATA[i][1], ARMOR_DATA[i][2] };
							if (items != null) {
...
/**
	...
	public static Dialogue warriorsGuildDialogue(Player player) {
		return new Dialogue() {
