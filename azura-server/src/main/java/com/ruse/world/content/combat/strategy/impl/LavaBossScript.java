...
public class LavaBossScript implements CombatStrategy {
    public int npcId() {
        return 8002;
    }
    public void regularAttack(Player player, NPC boss) {
        ...
        new Projectile(boss, player, 453, 75, 10, 110, 0, 0).sendProjectile();
    }
    ...
    public boolean customContainerAttack(Character entity, Character victim) {
        ...
        if (attack1) {
            boss.forceChat("You're all going to burn!!!");
            boss.performAnimation(new Animation(7769));
            sendRandomGraphics(boss, new Graphic(2039, 4), 12, 18);
            for (Player p: World.getNearbyPlayers(boss.getPosition(), 20)) {
                p.getPacketSender().sendCameraShake(3, 2, 3, 2);
                TaskManager.submit(new Task(6, p, false) {
                    @Override
                    public void execute() {
                        sendRandomGraphics(boss, new Graphic(2135), 12, 18);
        ...
        if (attack2) {
            boss.performAnimation(new Animation(7769));
            sendRandomGraphics(boss, new Graphic(453 , 4), 12, 18);
            ...
            for (Player p : World.getNearbyPlayers(boss.getPosition(), 20)) {
                ...
                TaskManager.submit(new Task(6, p, false) {
                    @Override
                    public void execute() {
                        sendRandomGraphics(boss, new Graphic(453 ), 12, 18);
                        p.getPacketSender().sendCameraNeutrality();
                        this.stop();
                    }
                });
