...
public final class RegionClipping {
	...
	public static boolean objectExists(GameObject object) {
		...
		boolean BeastChest3 = id == 415 && pos.getX() == 3090 && pos.getY() == 3514;
		/*boolean Sarcophagus = id == 6822 && pos.getX() == 3115 && pos.getY() == 3462;*/
		if (well || mageBankLever || lawAltar ||BeastChest3 || trees || chaosTunnels || lunar || barrows || rfd || lumbridgeCastle
				|| barbCourseRopeswing || catherbyAquariums || freeForAllPortal  || fightPit || godwars
	...
	public static GameObject getGameObject(Position position) {
		RegionClipping clipping = forPosition(position);
		if (clipping != null) {
			int x = position.getX();
			int y = position.getY();
			int height = position.getZ();
