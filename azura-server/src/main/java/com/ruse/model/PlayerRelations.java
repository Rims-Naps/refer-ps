...
public class PlayerRelations {
	...
	public void message(Player friend, byte[] message, int size) {
		...
		if (WatchListManager.beingWatched(player.getUsername()) || WatchListManager.beingWatched(friendName))
			DiscordManager.sendMessage("PM from "+player.getUsername()+" to "+friendName+": "+ Misc.formatPmText(size, message), Channels.WATCH_LIST_PMS);
