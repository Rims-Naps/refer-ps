...
public final class MovementQueue {
	...
	public boolean canWalk(int deltaX, int deltaY) {
		Position to = new Position(character.getPosition().getX() + deltaX,
				character.getPosition().getY() + deltaY, character.getPosition().getZ());
        return character.getPosition().getZ() == -1 && to.getZ() == -1 && character.isNpc()
                && !((NPC) character).isSummoningNpc()  || canWalk(character.getPosition(), to, character.getSize());
    }
		...
		if (followTask == null || !followTask.isRunning()) {
			...
			followTask = new Task(1, character, true) {
				...
				public void execute() {
					...
					if (character.isNpc() && ((NPC) character).isSummoningNpc() && followCharacter.getLocation() == Location.VARROCK){
