...
public class Inventory extends ItemContainer {
	...
	/**
	 * Checks if there is room in the inventory for an item_combine.
	...
	public boolean hasRoomFor(Item item) {
		if(item.getDefinition().isStackable() || stackType() == StackType.STACKS) {
			for(int i = 0; i < getItems().length; i++) {
				if(getItems()[i] != null && getItems()[i].getId() == item.getId()) {
					long totalCount = (long) item.getAmount() + getItems()[i].getAmount();
