...
public class Equipment extends ItemContainer {
	...
	public ItemContainer refreshItems() {
		if (getPlayer().viewingCosmeticTab) {
			getPlayer().getPacketSender().sendItemContainer(getPlayer().getCosmetics(), INVENTORY_INTERFACE_ID);
			getPlayer().getPacketSender().sendConfig(1882, 1);
		} else {
			getPlayer().getPacketSender().sendItemContainer(this, INVENTORY_INTERFACE_ID);
			getPlayer().getPacketSender().sendConfig(1882, 0);
		}
		return this;
	}
	...
	public static final int ASCEND = 11;
	public static final int AURA_SLOT = 6;
	public static final int WINGS= 8;

	public static int[] owner_items = new int[]{15792, 15793, 15794, 15795, 15796, 17122, 17124, 17123, 10724, 19944};
	public static int[] slots = new int[]{Equipment.HEAD_SLOT, Equipment.BODY_SLOT, Equipment.LEG_SLOT, Equipment.HANDS_SLOT, Equipment.FEET_SLOT, Equipment.AMULET_SLOT, Equipment.RING_SLOT, Equipment.CAPE_SLOT};
