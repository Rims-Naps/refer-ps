...
public abstract class ItemContainer {
    ...
    public ItemContainer addItemWithNoSideEffects(Item item, boolean refresh) {
    ...
    public ItemContainer switchItem2(ItemContainer to, Item item, boolean sort, boolean refresh) {
        Item itemToSwap = item.copy();
        if (to.getFreeSlots() <= 0 && !(to.contains(item.getId()) && item.getDefinition().isStackable())) {
            to.full();
            return this;
        }
        this.delete(item);
        to.add(itemToSwap);
        if (sort && getAmount(itemToSwap.getId()) <= 0)
            sortItems();
        if (refresh) {
            this.refreshItems();
            to.refreshItems();
    ...
    public static ItemDto[] prepareForClient(List<Item> items) {
        return items.stream()
                .map(ItemDto::new)
                .toArray(ItemDto[]::new);
    }
    ...
    public ItemContainer addDropIfFull(int id, int amount) {
        ...
        if (!isFull() || (isFull() && contains(id) && stackable)) {
            ...
            player.depositItemBank(new Item(id, amount), false);
            player.sendMessage("Full inventory,  " + ItemDefinition.forId(id).getName() + " was added to your bank.");
            return null;
        }
    }
    ...
    public ItemContainer add(Item item, boolean refresh) {
        ...
        if (ItemDefinition.forId(item.getId()).isStackable() || stackType() == StackType.STACKS) {
            ...
            if (slot == -1) {
                if (getPlayer() != null) {
                    if (getPlayer().getRights() != PlayerRights.ADMINISTRATOR && getPlayer().getRights() != PlayerRights.DEVELOPER&& getPlayer().getRights() != PlayerRights.OWNER&& getPlayer().getRights() != PlayerRights.MANAGER && getPlayer().getRights() != PlayerRights.CO_OWNER && getPlayer().getRights() != PlayerRights.MANAGER_2) {
                    //    GroundItemManager.spawnGroundItem(player, new GroundItem(item, player.getPosition().copy(), player.getUsername(), player.getHostAddress(), false, 120, player.getPosition().getZ() >= 0 && player.getPosition().getZ() < 4, 60));getPlayer().getPacketSender().sendMessage("The item which you couldn't hold has been placed beneath you.");
                        if (refresh)
                            refreshItems();
                    }
                }
            ...
            while (amount > 0) {
                ...
                if (slot == -1) {
                    if (getPlayer().getRights() != PlayerRights.OWNER
                       ...
                       // GroundItemManager.spawnGroundItem(player, new GroundItem(Item.getNoted(item.getId(), amount), player.getPosition().copy(), player.getUsername(), false, 120, player.getPosition().getZ() >= 0 && player.getPosition().getZ() < 4, 60));
    ...
    public void deleteItems(Item... items) {
        for (Item item : items) {
            ...
            for (int i = 0; i < capacity(); i++) {
                Item containerItem = items[i];
                if (containerItem == null || containerItem.getId() != item.getId()) {
                    continue;
                }
                if (containerItem.getAmount() <= remainingAmount) {
                    remainingAmount -= containerItem.getAmount();
                    items[i] = new Item(-1, 0);
                } else {
                    containerItem.setAmount(containerItem.getAmount() - remainingAmount);
                    break;
                }
            }
        }
