...
public class Bank extends ItemContainer {
	...
	public Bank open(Player player, boolean reset, boolean sort) {
        ...
        /**
         * Sets viewing interface after the checks
         */
        getPlayer().setViewing(Player.INTERFACES.BANK);
		if (sort)
			sortItems();
		...
		player.getPacketSender().sendConfig(115, player.withdrawAsNote() ? 0 : 1)
                ...
                .sendString(5383, "The Bank of " + GameSettings.RSPS_NAME);
		return this;
	}
	...
	public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if (to instanceof Inventory && getPlayer().withdrawAsNote()) {
			boolean canNote = def != null && def.isNoted();
			int freeSlots = to.getFreeSlots();
			if (!canNote && item.getAmount() > freeSlots) {
				getPlayer().getPacketSender()
						.sendMessage("You don't have enough room in your inventory.");
				return this;
			}
		}
	...
	public Bank refreshItems() {
        ...
        int amountToSend = bank.getValidItems().size() >= 51 ? 38 * (int) Math.ceil(((double) bank.getValidItems().size() / 10)) : 0;
        getPlayer().getPacketSender().setScrollBar(5385, amountToSend);
