...
public class NpcDefinition {
	...
	public static JsonLoader parseNpcs() {
		return new JsonLoader("./data/def/json/npc_definitions.json") {
			@Override
			public void load(JsonObject reader, Gson builder) {
				...
				if (reader.has("fire")) {
					definitions[index].setFire(reader.get("fire").getAsBoolean());
				}
				if (reader.has("water")) {
					definitions[index].setWater(reader.get("water").getAsBoolean());
				}
				if (reader.has("earth")) {
					definitions[index].setEarth(reader.get("earth").getAsBoolean());
				}

				definitions[index].boss = reader.has("boss") && reader.get("boss").getAsBoolean();
			}
		};
	}
	...
	public static int idForName(String name) {
		try {
			Optional<NpcDefinition> optionalNpcDefinition = Arrays.stream(definitions).filter(d -> d != null && d.name.equalsIgnoreCase(name)).findFirst();
			if (optionalNpcDefinition.isPresent()) {
				return optionalNpcDefinition.get().id;
			} else {
				System.out.println("No def found for name: "+name);
				return -1;
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * 
	 * @param combat the combat level to set.
	 */
	 ...
	 * 
	 * @param respawn the respawn time to set.
	 */
	 ...
	 * 
	 * @param attackAnim the attack animation to set.
	 */
	 ...
	 * 
	 * @param defenceAnim the defence animation to set.
	 */
	 ...
	 * 
	 * @param deathAnim the death animation to set.
	 */
