...
public class CeilingCollapseTask extends Task {
	...
	public void execute() {
		if (player == null || !player.isRegistered()
				|| player.getLocation() != Location.BARROWS || player.getLocation() == Location.BARROWS && player.getPosition().getY() < 8000) {
		...
		player.dealDamage(new Hit(30 + Misc.getRandom(20), Hitmask.NEUTRAL, CombatIcon.BLOCK));
	}
}
