...
public class VotingDMGBoostTask extends Task {
	...
	public VotingDMGBoostTask(Player player) {
	...
	public void execute() {
		...
		if (newMinutes < 0) {
			player.getPacketSender().sendMessage("<img=20> <col=330099>Your bonus x2 DMG has run out.");
			player.setMinutesVotingDMG(-1, false);
			stop();
		} else if (msg == 2 && newMinutes < 10) {
			//player.getPacketSender().sendMessage("<img=20> <col=330099>You have "
			//		+ Misc.format(player.getMinutesVotingDMG()) + " minutes of bonus x2 DMG left.");
		} else if (msg == 4) {
			//player.getPacketSender().sendMessage("<img=20> <col=330099>You have extra voting damage already!");
			msg = 0;
		}
		msg++;
	}

	public static void addBonusDMG(Player p, int minutes) {
