...
public class BonusExperienceTask extends Task {
	...
	public BonusExperienceTask(Player player) {
	...
	public void execute() {
		...
		if (newMinutes < 0) {
			player.getPacketSender().sendMessage("<img=22> <col=330099>Your bonus experience has run out.");
			player.setMinutesBonusExp(-1, false);
			stop();
		} else if (msg == 2 && newMinutes < 10) {
			player.getPacketSender().sendMessage("<img=22> <col=330099>You have "
	...
	public static void addBonusXp(Player p, int minutes) {
