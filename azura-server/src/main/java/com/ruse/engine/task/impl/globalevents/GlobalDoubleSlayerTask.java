...
public class GlobalDoubleSlayerTask extends Task {
    ...
    protected void execute() {
        if (timer < 1) {
            timer = 6000;
            World.sendMessage("BONUS Beast Tickets on Task Completion Event has Begun for 60 minutes!");
            GameSettings.DOUBLE_SLAYER = true;
        } else if(timer == 1) {
            World.sendMessage("BONUS Beast Tickets on Task Completion Event has Ended!");
